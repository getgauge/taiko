...
const _go = async (delta, options) => {
  ...
  if (
    ...
    !Object.hasOwn(options, "waitForNavigation")
  ) {
...
getPlugins().forEach((pluginName) => {
  ...
  for (const api in plugin) {
    const isApiOverridden = Object.hasOwn(overriddenAPIs, api);
    if (!isApiOverridden && Object.hasOwn(module.exports, api)) {
...
for (const func in module.exports) {
  ...
  if (realFuncs[func].constructor.name === "AsyncFunction") {
    module.exports[func] = async function () {
      ...
      // biome-ignore lint/complexity/noArguments: Cannot use optional arguments
