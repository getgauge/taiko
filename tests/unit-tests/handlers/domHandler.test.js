...
describe("domHandler", () => {
  ...
  before(() => {
    domHandler = rewire("taiko/lib/handlers/domHandler");
    calledWith = {};
  ...
  after(() => {
    ...
    domHandler
      ...
      .removeListener(
        "createdSession",
        domHandler.__get__("createdSessionListener"),
      );
  });

  it(".boundBox should give the bound  box of given node id", async () => {
    const box = await domHandler.boundBox(1);
