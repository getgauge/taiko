...
describe('Write and Clear', () => {
    ...
    describe('clear input field with wait for events', () => {
        let filePath;
        beforeAll(() => {
        ...
        test('test write and clear with wait for events()', async () => {
            await expect(inputField('Email').exists()).resolves.toBeTruthy();
            await write('abc@gmail.com', into(inputField('Email')));
            // await clear(inputField('Email'), {waitForEvents:['firstMeaningfulPaint']});
            await expect(inputField('Email').value()).resolves.toBe('');
        }, 30000);
    });
});