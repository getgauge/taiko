...
const { openBrowser, goto, evaluate, text, closeBrowser  } = require('../../lib/taiko');
...
describe(testName, () => {
    ...
    describe('with selector', () => {
        it('should return the result on the evaluation', async () => {
            let expected = 'Item 1';
            let actual = await evaluate(text('Item 1'), (element) => { return element.textContent.trim(); });
            expect(actual).to.be.equal(expected);
        });

        it('should pass args to the callback', async () => {
            let expected = 'Updated Item 1 with new item';
            await evaluate(text('Item 1'), (element, args) => {
