...
describe(test_name,()=>{
    let actualHeader, actualUrl, actualOptions, validateCalled = false;
    
    before(() => {
        const mockWrapper = async (options, cb) => { actualOptions = options; await cb(); };
        taiko.__set__('doActionAwaitingNavigation', mockWrapper);
        taiko.__set__('validate',() => {validateCalled = true;});
        taiko.__set__('network',{setExtraHTTPHeaders: (header) => { actualHeader = header;}});
        taiko.__set__('pageHandler',{handleNavigation: (url) => { actualUrl = url;}});
    });
    ...
    it('should call network.setExtraHTTPHeaders if header option is set',async () => {
        let options = {headers:{'Authorization':'Basic cG9zdG1hbjpwYXNzd29y2A=='}};
    ...
    it('should call doActionAwaitingNavigation with given options',async () => {
        let expectedOptions = {navigationTimeout:40000,waitForNavigation:false,waitForEvents:['firstMeaningfulPaint']};
