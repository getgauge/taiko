...
class BrowserFetcher {
    ...
    _resolveExecutablePath() {
        ...
        if (executablePath) {
            const missingText = !fs.existsSync(executablePath) ? 'Tried to use EXECUTABLE_PATH env variable to launch browser but did not find any executable at: ' + executablePath : null;
            return { executablePath, missingText };
        }
        const revisionInfo = this.revisionInfo(preferredRevision);
        const missingText = !revisionInfo.local ? 'Chromium revision is not downloaded. Provide EXECUTABLE_PATH or Install taiko again to download bundled chromium.' : null;
