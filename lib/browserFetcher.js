...
const preferredRevision = require(path.join(
  helper.projectRoot(),
  'package.json',
)).taiko.chromium_revision;
var url = require('url');
...
class BrowserFetcher {
  ...
  constructor(options = {}) {
    this._downloadsFolder =
      options.path ||
      path.join(helper.projectRoot(), '.local-chromium');
    ...
    assert(
      supportedPlatforms.includes(this._platform),
  ...
  waitForWSEndpoint(chromeProcess, timeout) {
    return new Promise((resolve, reject) => {
      ...
      const listeners = [
        ...
        helper.addEventListener(chromeProcess, 'exit', () =>
          onClose(),
        ),
        helper.addEventListener(chromeProcess, 'error', error =>
          onClose(error),
        ),
      ];
