...
const activeElement = async () => {
    ...
    for(const executionContextId of executionContextIds){
        ...
        if(activeElementDetails){
            ...
            if(!(!editable || disabled)) return {notWritable:false,nodeId: await evalCssSelector('document.activeElement',executionContextId),isPassword:activeElementDetails.type === 'password'}; 
        }
    }
