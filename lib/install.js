...
function onError(error) {
    console.error(`ERROR: Failed to download Chromium r${revision}! Set "TAIKO_SKIP_CHROMIUM_DOWNLOAD" env variable to skip download.`);
    console.error(error);
    process.exit(1);
}
...
function main() {
    if (process.env.TAIKO_SKIP_CHROMIUM_DOWNLOAD && process.env.TAIKO_SKIP_CHROMIUM_DOWNLOAD.toLowerCase() !== 'false') {
