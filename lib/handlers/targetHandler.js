...
let criTarget, browserDebugUrlTarget, currentBrowserContextId, currentTargetId;
...
const createdSessionListener = async (client, currentTarget) => {
  ...
  currentTargetId = currentTarget.id ? currentTarget.id : currentTarget;
...
const createBrowserContext = async (url) => {
  ...
  currentBrowserContextId = browserContextId;
...
const createTarget = async (url) => {
  let openWindowOptions = {
    url,
    browserContextId: currentBrowserContextId,
  };
...
const getCriTargets = async function (identifier) {
  ...
  if (!identifier) {
    return {
      matching: pages.filter((target) => target.id === currentTargetId),
      others: pages.filter((target) => target.id !== currentTargetId),
    };
  }
