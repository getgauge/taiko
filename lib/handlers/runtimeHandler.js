...
const activeElement = async () => {
    ...
    for (const executionContextId of executionContextIds) {
        ...
        if (activeElementDetails) {
            editable = (!activeElementDetails.readOnly) && (['INPUT', 'TEXTAREA', 'SELECT'].includes(activeElementDetails.tagName) || (activeElementDetails.isContentEditable));
...
module.exports = { findElements, activeElement, getNodeIdsFromResult, runtimeEvaluate, runtimeCallFunctionOn };
