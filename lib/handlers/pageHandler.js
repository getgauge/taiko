...
const emitFrameNavigationEvent = p => {
  if (
    !(frameNavigationPromise && frameNavigationPromise[p.frameId])
  ) {
...
const isSameUrl = (url1, url2) => {
  ...
  if (url1.protocol === 'file:' && url2.protocol === 'file:') {
    if (
...
const handleNavigation = async url => {
  ...
  let urlToNavigate = normalizeAndHandleRedirection(
    nodeURL.parse(url).href,
  );
  const handleRequest = request => {
    ...
    logEvent(
      `Request Started. Handling requests, requestUrl ${requestUrl}, urlToNavigate ${urlToNavigate}`,
    );
  ...
  try {
    logEvent(`Navigating to url ${url}`);
...
const captureScreenshot = async function(
  domHandler,
  selector,
  options,
) {
  ...
  if (isSelector(selector)) {
    if (options.fullPage)
      console.warn(
        'Ignoring fullPage screenshot as custom selector is found!',
      );
    let padding = options.padding || 0;
    let elems = await elements(selector);
    const { x, y, width, height } = await domHandler.boundBox(
      elems[0],
    );
    clip = {
      x: x - padding,
