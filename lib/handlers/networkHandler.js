    ...
    if (!interceptor.action) {
      ...
      p.respond = mock => {
        ...
        network.continueInterceptedRequest(options).catch(() => {
          console.warn(
            `Could not intercept request ${p.request.url}`,
          );
        });
      };
...
const mockResponse = (response, options) => {
  let responseBodyJson = isObject(response.body)
