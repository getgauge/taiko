...
xhrEvent.on('createdSession',(client) => {
    dom = client.DOM;
});
...
const calculateNewCenter = async (sourceElem, destElem) => {
    const sourceBoundary = await getBoundingClientRect(sourceElem);
    ...
    return { y: (newBoundary.top + newBoundary.bottom) / 2, x: (newBoundary.left + newBoundary.right) / 2, newBoundary: newBoundary };
};


module.exports = { boundBox, boundingBoxCenter, getBoundingClientRect, getPositionalDifference, getBoxModel, calculateNewCenter };