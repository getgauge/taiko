...
const checkIfActionable = async (elem, checks) => {
  ...
  for (let check of checks) {
    actionable = await checksMap[check].predicate(elem);
    if (!actionable) {
      error = checksMap[check].error;
      break;
    }
  }
...
const waitAndGetActionableElement = async (
  selector,
  checks = ['visible', 'disabled'],
  args = [],
) => {
  const elements = await handleRelativeSearch(await findElements(selector), args);
  let elementsLength = elements.length;
  if (elementsLength > defaultConfig.elementsToMatch) {
    elements.splice(defaultConfig.elementsToMatch, elements.length);
  }
...
module.exports = { description, waitAndGetActionableElement, checkIfElementIsNotCovered };
