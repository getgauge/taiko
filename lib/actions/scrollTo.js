...
const scrollToElement = async (element, alignmentsOptions = {}) => {
  function scrollToNode() {
    const element = this.nodeType === Node.TEXT_NODE ? this.parentElement : this;
    let blockAlign = 'nearest';
    let inlineAlign = 'nearest';
    if (
      typeof alignmentsOptions !== 'undefined' &&
      typeof alignmentsOptions.alignments !== 'undefined'
    ) {
      blockAlign = alignmentsOptions.alignments.block || blockAlign;
      inlineAlign = alignmentsOptions.alignments.inline || inlineAlign;
    }
    element.scrollIntoView({ block: blockAlign, inline: inlineAlign });
    return 'result';
  }
