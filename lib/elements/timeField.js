...
class TimeField extends Element {
  ...
  async select(dateTime) {
    ...
    function selectDate(value) {
      ...
      const setAndDispatchEvent = (self, value) => {
        self.value = value;
      ...
      const dateFormat = {
        ...
        "datetime-local": `${value.getFullYear()}-${`0${value.getMonth() + 1}`.slice(-2)}-${`0${value.getDate()}`.slice(-2)}T${`0${value.getHours()}`.slice(-2)}:${`0${value.getMinutes()}`.slice(-2)}`,
      ...
      const dateTimeValue = validate(this, value)
