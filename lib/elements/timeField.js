  ...
  async select(dateTime) {
    ...
    function selectDate(value) {
      ...
      const validationDateFormat = {
        ...
        time: () => {
          return {
            max: self.max ? new Date(`2000-09-08T${self.max}`) : null, //dummy date for time field to validate min max time
            min: self.min ? new Date(`2000-09-08T${self.min}`) : null,
          };
        },
      };
      ...
      let validate = function (self, value) {
        const { min, max } = validationDateFormat[self.type]
          ? validationDateFormat[self.type]()
          : { min: self.min, max: self.max };
        if (max && min) {
          return new Date(max) >= value && new Date(min) <= value;
        }
        if (max) {
          return new Date(max) >= value;
        }
        if (min) {
          return new Date(min) >= value;
        }
        return true;
      };
    ...
    await doActionAwaitingNavigation(navigationOptions, async () => {
      const options = { nodeId: this.get(), arg: dateTime, returnByValue: true };
