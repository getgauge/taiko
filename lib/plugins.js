...
const getExecutablePlugins = () => {
    let pluginsGlobalPath = childProcess.spawnSync('npm', ['root', '-g'], { shell: true }).stdout.toString().trim();
    let pluginsLocalPath = childProcess.spawnSync('npm', ['root'], { shell: true }).stdout.toString().trim();
    ...
    return fs.readdirSync(pluginsPath)
        .filter(file => {
            ...
            if (npmModule.isSymbolicLink()) {
                let fsStat = fs.statSync(path.resolve(pluginsPath, fs.readlinkSync(path.resolve(pluginsPath, file))));
                console.log(`file  : ${file}, fsStat.isDirectory() : ${fsStat.isDirectory()} file.match(/^taiko-.*/): ${file.match(/^taiko-.*/)}`);
