...
const getExecutablePlugins = () => {
  ...
  let globalExecutablePlugin = filterExecutablePlugin(
    globalPlugins,
    pluginsGlobalPath,
  );
  let localExecutablePlugin = filterExecutablePlugin(
    localPlugins,
    pluginsLocalPath,
  );
  ...
  return Object.keys(globalExecutablePlugin).reduce(
    (localPlugins, globalPlugin) => {
      if (
        ...
        localPlugins[globalPlugin] =
          globalExecutablePlugin[globalPlugin];
      return localPlugins;
    },
...
function filterExecutablePlugin(plugins, pluginsPath) {
  return plugins
    .filter(npmModule => {
      let packageJson = getPackageJsonForPlugin(
        pluginsPath,
        npmModule,
      );
      return (
        packageJson.capability &&
        packageJson.capability.includes('subcommands')
      );
    })
    .reduce((plugins, plugin) => {
      plugins[plugin.replace(/^taiko-/, '')] = path.join(
        pluginsPath,
        plugin,
      );
      return plugins;
