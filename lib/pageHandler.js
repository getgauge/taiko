...
const handleNavigation = async (url, action) => {
    let responsePromise = new Promise((resolve) => {
        xhrEvent.once('responseReceived', (response) => {
            const responseUrl = handleUrlRedirection(response.url).split('://')[1].replace(/^\/*/gm,'');
            const urlToNavigate = handleUrlRedirection(url).split('://')[1].replace(/^\/*/gm,'');
            if(responseUrl === urlToNavigate)  resolve(response);
        });
    });
