    ...
    for (const key in options) {
        if (Object.prototype.hasOwnProperty.call(defaultConfig, key)) {
            if (typeof defaultConfig[key] !== typeof options[key])
                throw new Error(`Invalid value for ${key}. Expected ${typeof defaultConfig[key]} received ${typeof options[key]}`);
            defaultConfig[key] = options[key];
        }
        else {
            throw new Error(`Invalid config ${key}. Allowed configs are ${Object.keys(defaultConfig).join(', ')}`);
        }
    }
};
