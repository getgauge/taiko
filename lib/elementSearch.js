...
async function matchTextNode(
  ...
  return options ? await $$xpath(
    `//text()[translate(normalize-space(string()), "${textToTranslate}", "${translateTo}")=${xpathText}]`,options.selectHiddenElement
  ) : await $$xpath(
...
async function containsTextNode(
  ...
  return options ? await $$xpath(
    `//text()[contains(translate(normalize-space(string()), "${textToTranslate}", "${translateTo}"), ${xpathText})]`, options.selectHiddenElement
  ) : await $$xpath(
...
async function matchValueOrType(
  ...
  return options ? await $$xpath(
    '//' +
      tagName +
      `[translate(normalize-space(@value), "${textToTranslate}", "${translateTo}")=${xpathText} or translate(normalize-space(@type), "${textToTranslate}", "${translateTo}")=${xpathText}]`,options.selectHiddenElement
  ) : await $$xpath(
      '//' +
      tagName +
...
async function matchTextAcrossElements(
  ...
  return options ? await $$xpath(
    '//' +
      tagName +
      `[not(descendant::*[${xpathToTranslateInnerText}=${xpathText}]) and ${xpathToTranslateInnerText}=${xpathText}]`, options.selectHiddenElement
  ) : await $$xpath(
      '//' +
      tagName +
...
const containsTextInChildElements = async function (
  ...
  xpathText,options
   ...
   return options ? await $$xpath(
    '//' +
    tagName +
    `[contains(translate(normalize-space(.), "${textToTranslate}", "${translateTo}"), ${xpathText})]`,options.selectHiddenElement
  ) : $$xpath(
       '//' +
       tagName +
