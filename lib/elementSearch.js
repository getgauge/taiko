...
function match(text, options = {}, ...args) {
  ...
  const get = async (tagName = '*') => {
    ...
    const textSearch = function (args) {
      ...
      exactMatchXpath = args.tagName === '*' ? `//text()[translate(normalize-space(string()), "${textToTranslate}", "${translateTo}")=${xpathText}]| ${exactMatchXpath}` : exactMatchXpath;
      ...
      containsXpath = args.tagName === '*' ? `//text()[contains(translate(normalize-space(string()), "${textToTranslate}", "${translateTo}"), ${xpathText})]| ${containsXpath}` : containsXpath;
      ...
      if(args.tagName === 'button' && !matchingElements.length) return iterateResultForNodes(evalXpath(containsButtonTextInChild));
