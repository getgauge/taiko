...
function match(text, eventEmitter, ...args) {
  assertType(text);
  const get = async (e = '*') => {
    let elements;
  ...
  return {
    get: async (tag, retryInterval, retryTimeout) => {
      ...
      return await getIfExists(get,description, eventEmitter)(tag, retryInterval, retryTimeout);
    },
    description,
...
const element = async (selector, tag) =>
  await (await elements(selector, tag))[0].get();
...
const elements = async (selector, tag) => {
  const elements = await (async () => {
    if (isString(selector)) {
      ...
      return selector.elements ? selector.elements(tag) : [await selector.get()];
    }
    return null;
  })();
  if (!elements || !elements.length) {
