...
function match(text, options = {}, ...args) {
  ...
  const get = async (tagName = '*') => {
    ...
    const textSearch = function (args) {
      ...
      const xpathText = `translate(normalize-space(${convertQuotes(args.text, )}), "${textToTranslate}", "${translateTo}")`;
      ...
      let matchingElements = nodesSnapshot.snapshotLength ? iterateNodesForResult(nodesSnapshot) : iterateNodesForResult(evalXpath(exactMatchAcrossElement));
      if(args.exactMatch || matchingElements.length) return matchingElements;
      
      //find contains
      nodesSnapshot = evalXpath(containsXpath);
      matchingElements = nodesSnapshot.snapshotLength ? iterateNodesForResult(nodesSnapshot) : iterateNodesForResult(evalXpath(containsMatchAcrossElement));
 
      if(args.tagName === 'button' && !matchingElements.length) return iterateNodesForResult(evalXpath(containsButtonTextInChild));
