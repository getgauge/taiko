...
function match(text, options = {}, ...args) {
  ...
  const get = async (tagName = '*') => {
    ...
    const textSearch = function (args) {
      ...
      function iterateResultForNodes(nodesSnapshot) {
        let result = [];
      ...
      const xpathText = `translate(normalize-space(${xpath(args.text, )}), "${textToTranslate}", "${translateTo}")`;
      const exactMatchXpath = `//text()[translate(normalize-space(string()), "${textToTranslate}", "${translateTo}")=${xpathText}]|
      //${args.tagName}[translate(normalize-space(@value), "${textToTranslate}", "${translateTo}")=${xpathText} or translate(normalize-space(@type), "${textToTranslate}", "${translateTo}")=${xpathText}]`;
      ...
      const containsXpath = `//text()[contains(translate(normalize-space(string()), "${textToTranslate}", "${translateTo}"), ${xpathText})]|
      //${args.tagName}[contains(translate(normalize-space(@value), "${textToTranslate}", "${translateTo}"), ${xpathText}) or contains(translate(normalize-space(@type), "${textToTranslate}", "${translateTo}"), ${xpathText})]`;
      ...
      let matchingElements = nodesSnapshot.snapshotLength ? iterateResultForNodes(nodesSnapshot) : iterateResultForNodes(evalXpath(exactMatchAcrossElement));
      if(args.exactMatch || matchingElements.length) return matchingElements;
      
      //find contains
      nodesSnapshot = evalXpath(containsXpath);
      matchingElements = nodesSnapshot.snapshotLength ? iterateResultForNodes(nodesSnapshot) : iterateResultForNodes(evalXpath(containsMatchAcrossElement));
      return matchingElements;
    };
    
    elements = Element.create(
      await runtimeHandler.findElements(textSearch, { text: text, exactMatch: options.exactMatch, tagName: tagName }),
      runtimeHandler
    );
