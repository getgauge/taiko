...
function match(text, options = {}, ...args) {
  ...
  const get = async (tagName = '*') => {
    ...
    const textSearch = function (selectorElement, args) {
      ...
      function checkIfRegexMatch(text, exactMatch) {
        return exactMatch
      ...
      function checkIfChildHasMatch(childNodes, exactMatch) {
        ...
        if (childNodes.length) {
          for (let childNode of childNodes) {
            const nodeTextContent = getNormalizedText(childNode.textContent);
            if (
              exactMatch &&
              (checkIfRegexMatch(childNode.textContent, true) || nodeTextContent === searchText)
            ) {
              return true;
            }
            if (
              checkIfRegexMatch(childNode.textContent, false) ||
      ...
      while ((node = iterator.nextNode())) {
        const nodeTextContent = getNormalizedText(node.textContent);

        //Match values and types for Input and Button nodes
        if (node.nodeName === 'INPUT') {
          const nodeValue = getNormalizedText(node.value);
          if (
            // Exact match of values and types
            checkIfRegexMatch(node.value, true) ||
            ...
            !args.exactMatch &&
            (checkIfRegexMatch(node.value, false) ||
              (!isRegex(searchText) &&
        ...
        if (checkIfRegexMatch(node.textContent, true) || nodeTextContent === searchText) {
          ...
          !args.exactMatch &&
          (checkIfRegexMatch(node.textContent, false) ||
            (!isRegex(searchText) && nodeTextContent.includes(searchText)))
        ) {
