const {xpath,assertType,getIfExists,exists,isString} = require('./helper');
let runtimeHandler, handleRelativeSearch,descEventHandler;

function initElementSearch(runtime,relativeSearch,descEvent) {
    runtimeHandler = runtime;
    handleRelativeSearch = relativeSearch;
    descEventHandler = descEvent;
}
...
function match(text, ...args) {
    ...
    return { get: getIfExists(get), exists: exists(getIfExists(get), descEventHandler), description: `Element matching text "${text}"`};
}
...
const isElementVisible = async (nodeId) => {
    ...
    return !((await runtimeHandler.runtimeCallFunctionOn(isHidden, null,
        { nodeId: nodeId, returnByValue: true })).result.value);
};

const $$ = async (selector) => {
    return (await filterVisibleNodes(await runtimeHandler.findElements(`document.querySelectorAll('${selector}')`)));
};
...
module.exports = {match,initElementSearch,$$xpath,$$,element,elements,isSelector,isElementVisible};
