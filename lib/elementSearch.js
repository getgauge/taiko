...
const isElementDisabled = async nodeId => {
  function isDisabled() {
  ...
  return (
    await runtimeHandler.runtimeCallFunctionOn(isDisabled, null, {
      nodeId: nodeId,
      returnByValue: true,
    })
  ).result.value;
...
const $$ = async selector => {
  const elements = await filterVisibleNodes(
    await runtimeHandler.findElements(`document.querySelectorAll('${selector}')`),
  );
  return Element.create(elements, runtimeHandler);
};
...
const $$xpath = async (selector, selectHiddenElements) => {
  ...
  return Element.create(
    selectHiddenElements ? elements : await filterVisibleNodes(elements),
    runtimeHandler,
  );
};
