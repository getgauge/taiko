...
const networkPresets = require('./data/networkConditions').default;
let requestPromises;
const defaultErrorReason = 'Failed';
let interceptors = [];
let network, xhrEvent, logEvent, navigationTracker;

const setNetwork = async (n, event, eventLogger) => {
    network = n;
    xhrEvent = event;
...
const registerInterceptHandler = () => {
    ...
    network.responseReceived((p) => {
        if( p.response.status.toString().match(/(4|5)\d+/) ){
            navigationTracker.failed(p.response.url, `HTTP status code : ${p.response.status}, ${p.response.statusText}`);
        }
    });
};
...
const emitXHREvent = p => {
    if (!(requestPromises && requestPromises[p.requestId])) {
        logEvent('Request started: ' + p.requestId);
        let resolve;
...
const setNetworkEmulation = async (networkType) => {
    ...
    if (emulate === undefined) 
        throw new Error(`Please set one of the given network types ${networkModes}`);
    await network.emulateNetworkConditions(emulate).catch(err => {
        console.warn(`Could not emulate network ${err}`);
    });
};

const ensureNavigationSeccedsFor = async (_navigationTracker) => {
...
module.exports = { setNetwork, addInterceptor, resetInterceptors, setNetworkEmulation, ensureNavigationSeccedsFor };