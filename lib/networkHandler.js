...
const handleInterceptor = p => {
    ...
    if(matchesLen){
        ...
        if(matchesLen > 1) console.warn(`WARNING: More than one intercept found for request. Applying "${interceptor.requestUrl}" with action ${interceptor.action.toString()}`);
...
module.exports = { setNetwork, addInterceptor, resetInterceptors };