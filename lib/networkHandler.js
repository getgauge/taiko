...
const setNetworkEmulation = async (method, params, criTarget) => {
    const targets = (await criTarget.getTargets()).targetInfos;
    const targetInfo = await targets.find((item) => item.attached === true);
    const { sessionId } = await criTarget.attachToTarget({targetId: targetInfo.targetId});
    const id = 7; // random id
    const message = JSON.stringify({id, method, params});
    await console.log(message);
    await criTarget.sendMessageToTarget({sessionId, message}).catch(err => {
