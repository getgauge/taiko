...
const handleInterceptor = p => {
    ...
    for(const interceptor of interceptors){
        let matched = true;
        for (const key in interceptor['request']) {
            if (!p['request'][key]) throw new Error(`There is no matching request attribute for "${key}"`);
            if (!(p['request'][key] === interceptor['request'][key])) matched = false;
        }
        if (matched){ 
            if(interceptor.callback) interceptor.mock = {response:interceptor.callback(p)};
            if(interceptor.mock.response) options = mockResponse(interceptor.mock.response, options);
            else options = overrideRequest(interceptor.mock, options);
        }    
    }
