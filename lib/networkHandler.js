...
const networkPresets = require('./networkConditions').default;
...
const setNetworkEmulation = async (networkType) => {
    const emulate = await _.head(_.filter(networkPresets, {mode: networkType}));
    let networkMode = []; 
    _.filter(networkPresets,
        (p) => {return networkMode.push(p.mode);});
    if (emulate === undefined) 
        throw new Error(`Please set one of the given network types ${networkMode}`);
    await network.emulateNetworkConditions(emulate.value).catch(err => {
...
module.exports = { setNetwork, addInterceptor, resetInterceptors, setNetworkEmulation };