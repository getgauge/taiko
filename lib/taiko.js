...
module.exports.openBrowser = async (options = { headless: true }) => {
    ...
    plugins.forEach((handler) => handler(module.exports));
 ...
 * @returns {Promise<Object>} - Object with the description of the action performed.
 */
module.exports.openTab = async (targetUrl, options = { timeout: default_timeout, waitForEvents : ['firstMeaningfulPaint'] }) => {
 ...
 */
module.exports.goto = async (url, options = { timeout: default_timeout, waitForEvents : ['firstMeaningfulPaint']}) => {
    validate();
 ...
 * @returns {Promise<Object>} - Object with the description of the action performed and the final URL.
 */
module.exports.reload = async (url, options = { timeout: default_timeout, waitForEvents : ['firstMeaningfulPaint'] }) => {
    validate();
    // const promises = setNavigationPromises(options);
    options = setNavigationOptions(options);
    await doActionAwaitingNavigation(options, async () => {
 ...
 * @returns {Promise<Object>} - Object with the description of the action performed.
 */
module.exports.goBack = async (options = { timeout: default_timeout, waitForEvents : ['firstMeaningfulPaint'] }) => {
    validate();
    await _go(-1, options);
 ...
 * @returns {Promise<Object>} - Object with the description of the action performed.
 */
module.exports.goForward = async (options = { timeout: default_timeout, waitForEvents : ['firstMeaningfulPaint'] }) => {
    validate();
    await _go(+1, options);
...
module.exports.button = (attrValuePairs, ...args) => {
    validate();
    const selector = getValues(attrValuePairs, args);
    const getBySelector = getElementGetter(selector,
        async () => await $$xpath(`//input[@type='submit' or @type='reset' or @type='button'][@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)] | //label[contains(string(), ${xpath(selector.label)})]/input[@type='submit' or @type='reset' or @type='button']`),
        'input');

    const getByName = getElementGetter(selector,
        async () => await $$xpath('//input[@type="submit" or @type="button" or @type="reset"]'), 'input');

    const get = async () => {
        const button = await getBySelector();
        if (button.length) return button;
        let element = await $$xpath(`//input[contains(@value, ${xpath(selector.label)})]`);
        if (element.length) return getByName();
        return await getElementGetter(selector, async () => await elements(selector.label, 'button'), 'button');
    };
...
module.exports.radioButton = (attrValuePairs, ...args) => {
    ...
    const selector = getValues(attrValuePairs, args);
    const get = getElementGetter(selector,
        async () => await $$xpath(`//input[@type='radio'][@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)] | //label[contains(string(), ${xpath(selector.label)})]/input[@type='radio']`),
        'input[type="radio"]');
