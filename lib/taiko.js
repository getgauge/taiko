...
const doActionAwaitingNavigation = async (options, action) => {
  ...
  try {
    await action();
    logEvent('Waiting for load to start');
    await waitForPromises(promises, options.waitForStart);
    logEvent(
      `Waiting for navigation. Found ${promises.length} promises`,
    );
    ...
    logEvent(`Navigation failed ${e}`);
    throw e;
  } finally {
    logEvent('Cleaning up listeners added');
