...
const setOptions = (options, x, y) => {
    ...
    options.elemetsToMatch = options.elemetsToMatch || 10;
    return options;
};
...
const checkIfElementIsCovered = async (elem, x, y, elems, selector) => {
    let isElemAtPoint = await checkIfElementAtPointOrChild(elem, parseInt(x), parseInt(y));
 ...
 * @param {number} [options.elemetsToMatch=10] - Number of elements to loop through to match the element with given selector.
 * @returns {Promise<Object>} - Object with the description of the action performed.
 */
...
async function click(selector, options = {}, ...args) {
    ...
    if (elemsLength > options.elemetsToMatch){
        elems.splice(options.elemetsToMatch, elems.length);
    }
    for (let elem of  elems){
        ...
        isElemAtPoint = await checkIfElementIsCovered(elem, x, y, elems, selector);
    ...
    await doActionAwaitingNavigation(options.waitForStart, options.waitForNavigation, options.timeout, async () => {
...
module.exports.write = async (text, into, options = { delay: 10 }) => {
    ...
    if (into) {
        ...
        if (await _getActiveElementTag() !== 'BODY') {
            await _write(text, options);
            return { description: `Wrote ${text} into the focused element.` };
        }
        throw new Error('Not focused into a writable field. Please use "write(\'admin\', into(\'Username:\'))" or increse the delay.');
    }
};

const _getActiveElementTag = async () => {
    const result = await runtime.evaluate({
...
const doActionAwaitingNavigation = async (waitForStart, awaitNavigation, timeout, action) => {
    var promises = [];
    let func = addPromiseToWait(promises);
    xhrEvent.addListener('xhrEvent', func);
    const pushLoadEvent = waitForLoadEvent(promises);
    xhrEvent.once('frameStartedLoading', pushLoadEvent);
    await action();
    await waitForPromises(promises, waitForStart);
    if (awaitNavigation) {
        await waitForNavigation(timeout, promises).catch(handleTimeout(func, timeout));
    }
    xhrEvent.removeListener('xhrEvent', func);
};
...
const waitForLoadEvent = (promises) => {
    return () => {
        promises.push(page.loadEventFired());
...
const handleTimeout = (func, options) => {
    return (e) => {
        xhrEvent.removeListener('xhrEvent', func);
        if (e === 'Timedout')
            throw new Error(`Navigtion took more than ${options.timeout}ms. Please increase the timeout.`);
    };
};
...
const filter_visible_nodes = async (nodeIds) => {
    ...
    function isHidden() {
        return this.offsetParent === null || (window.getComputedStyle(this).getPropertyValue('opacity') < 0.1);
    }
