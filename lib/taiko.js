...
module.exports.openWindow = async (url, options = {}) => {
  if (!browserMode && options.incognito) {
    throw new Error(
      'OpenWindow in Incognito works only with headless browser.',
    );
  }
  browser = await connect_to_cri(browserDebugUrl, { window: true });
  const targetId = await createBrowserContext(browser, url, options);
...
module.exports.closeWindow = async arg => {
  //Fail if arg is not object
  await closeBrowserContext(arg);
  _client = null;
  descEvent.emit('success', `Window with name ${arg.name} closed`);
};
