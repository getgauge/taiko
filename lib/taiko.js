...
module.exports.dragAndDrop = async (source,destination) =>{
    ...
    let destElem = await element(destination);
    let sourcePosition = await domHandler.boundingBoxCenter(sourceElem);
    let destPosition = await domHandler.boundingBoxCenter(destElem);
    let options = setOptions({}, sourcePosition.x,sourcePosition.y);
    await doActionAwaitingNavigation(options, async () => {
        await dragAndDrop(options,destPosition); 
    });
    return {description: `Dragged and dropped ${description(source,true)} to ${description(destination,true)}}`};
};

const dragAndDrop = async (options,destPosition) =>{
    options.type = 'mouseMoved';
    await input.dispatchMouseEvent(options);
    options.type = 'mousePressed';
    await input.dispatchMouseEvent(options);
