...
const {
    helper, wait, isString, isFunction, getIfExists, waitUntil,
    exists, xpath, waitForNavigation, timeouts, assertType
} = require('./helper');
...
const calculateDestPosition = async (sourceElem, destElem) => {
    ...
    if (defaultConfig.headful) {
        await overlay.highlightQuad({
            quad: [newBoundary.right, newBoundary.top, newBoundary.right, newBoundary.bottom, newBoundary.left, newBoundary.bottom, newBoundary.left, newBoundary.top],
...
const selectorText = get => {
    return async () => {
        ...
        for (const elem of elems) {
            texts.push((await evaluate(elem, function getText() {
                return this.innerText;
            })).value);
        }
        return texts;
