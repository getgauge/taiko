...
const connect_to_cri = async () => {
    return new Promise(function connect(resolve) {
        cri(async (c) => {
            ...
            networkHandler.setNetwork(network,xhrEvent);
 ...
 */
module.exports.goto = async (url, credentials, options = { timeout: default_timeout }) => {
    validate();
    ...
    if(credentials){
        if(credentials.username && credentials.password) await networkHandler.setCredentials(credentials);
        else options = credentials;
    }
...
/**
 * Creats authentication object from username and password
 * 
 ...
 * @param {object}
 */
module.exports.authenticate = (username,password) =>{ return({username,password}); };
...
const waitForNavigation = (timeout, promises = []) => {
    ...
    return Promise.race([Promise.all(promises), waitFor(timeout)]);
};
...
module.exports.metadata = {
    ...
    'Page actions': ['goto', 'authenticate', 'reload', 'title', 'click', 'doubleClick', 'rightClick', 'hover', 'focus', 'write', 'attach', 'press', 'highlight', 'scrollTo', 'scrollRight', 'scrollLeft', 'scrollUp', 'scrollDown', 'screenshot'],
