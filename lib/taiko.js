...
module.exports.openIncognitoWindow = async url => {
  if (!browserMode) {
    console.warn('Recommend open Incognito window in headless mode');
  }
  browser = await connect_to_cri(browserDebugUrl, { profiles: true });
  const targetId = await createBrowserContext(browser, url);
  await connect_to_cri(targetId);
  await pageHandler.handleNavigation(url);
  await dom.getDocument();
  descEvent.emit(
    'success',
    `Incognito window opened with url ${url}`,
  );
};

module.exports.closeIncognitoWindow = async url => {
  await closeBrowserContext(url);
  _client = null;
  descEvent.emit('success', `Incognito window with ${url} closed`);
};

const _switchToIncognitoBrowser = async url => {
  await switchBrowserContext(connect_to_cri, url);
  await dom.getDocument();
  descEvent.emit('success', `Switched to Incognito window ${url}`);
};
