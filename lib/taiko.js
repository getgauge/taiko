...
async function mouseAction(
  ...
  if (
    ...
    throw Error(
      'Kindly pass valid selector/string, Invalid Selector value passed : ' +
        selector,
    );
  }
  if (!actions.includes(selector)) {
    ...
    let {
      ...
      left,
      right,
    } = await domHandler.getBoundingClientRect(elem);
    coordinates.x = coordinates.x + parseInt(left);
    coordinates.y = coordinates.y + parseInt(top);
