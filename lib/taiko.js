...
const { getPlugins, registerHooks, pluginHooks } = require('./plugins');
let page,
  network,
  input,
  _client,
...
const initCRI = async (target, n, options = {}) => {
  try {
    const { target, options } = await pluginHooks.preConnectionHook(target, options);
    var c = await cri({
      target,
...
const connect_to_cri = async (target, options = {}) => {
  ...
  var tgt =
    targetWSDebugUrl || target || (await targetHandler.waitForTargetToBeCreated(numRetries));
...
module.exports.switchTo = async (arg) => {
  ...
  if (isObject(arg) && !isRegex(arg) && !isString(arg)) {
    ...
    targetId = targets.matching[0].id;
