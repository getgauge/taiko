...
module.exports.goto = async (url, options = { timeout: default_timeout, waitForEvents: ['firstMeaningfulPaint'] }) => {
    ...
    options = setNavigationOptions(options);
    let navigationSuccessTracker = new NavigationSuccessTracker(url);
    await doActionAwaitingNavigation(options, async () => {
        networkHandler.ensureNavigationSeccedsFor(navigationSuccessTracker);
        const res = await page.navigate({ url: url });
        if( !navigationSuccessTracker.hasSuccedded())
            throw new Error(`Navigation to url ${url} failed.\n REASON: ${navigationSuccessTracker.getReason()}`);
...
class NavigationSuccessTracker {
    constructor(url) {
        this.url = url;
        this.reason = null;
    }
...
async function click(selector, options = {}, ...args) {
    ...
    if (isSelector(selector) || isString(selector) || Number.isInteger(selector)){
        ...
        await waitForMouseActions(options);
        return { description: `Clicked on coordinates x : ${selector.x} and y : ${selector.y}`};
    }
}
...
async function mouseAction(action, coordinates, options = {}) {
    ...
    return { description: `Performed mouse ${action} action at {${coordinates.x},${coordinates.y}}`};
}
