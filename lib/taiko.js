...
module.exports.emitter = xhrEvent;
...
module.exports.openBrowser = async (options = { headless: true }) => {
    ...
    xhrEvent.emit('success', description);
};
...
module.exports.closeBrowser = async () => {
    ...
    xhrEvent.emit('success', 'Browser closed');
};
...
module.exports.switchTo = async (targetUrl) => {
    ...
    await dom.getDocument();
    xhrEvent.emit('success', 'Switched to tab with url ' + targetUrl);
};
...
module.exports.intercept = async (requestUrl, option) => {
    ...
    xhrEvent.emit('success', 'Interceptor added for ' + requestUrl);
};
...
module.exports.emulateNetwork = async (networkType) => {
    ...
    xhrEvent.emit('success', 'Set network emulation with values ' + JSON.stringify(networkType));
};
...
async function emulateDevice(deviceModel) {
    ...
    xhrEvent.emit('success', 'Device emulation set to ' + deviceModel);
}
...
module.exports.setViewPort = async (options) => {
    ...
    xhrEvent.emit('success', 'ViewPort is set to width ' + options.width + 'and height ' + options.height);
};
...
module.exports.openTab = async (targetUrl, options = { navigationTimeout: config.navigationTimeout, waitForEvents: ['firstMeaningfulPaint'] }) => {
    ...
    xhrEvent.emit('success', 'Opened tab with url ' + targetUrl);
};
...
module.exports.closeTab = async (targetUrl) => {
    ...
    if (!targetToConnect) {
        await _closeBrowser();
        xhrEvent.emit('success', 'Closing last target and browser.');
    }
    client.removeAllListeners();
    client = null;
    ...
    await dom.getDocument();
    xhrEvent.emit('success', 'Closed tab with url ' + targetUrl);
};
...
module.exports.overridePermissions = async (origin, permissions) => {
    ...
    xhrEvent.emit('success', 'Override permissions with ' + permissions);
};
...
module.exports.clearPermissionOverrides = async () => {
    ...
    xhrEvent.emit('success', 'Cleared permission overrides');
};
...
module.exports.setCookie = async (name, value, options = {}) => {
    ...
    xhrEvent.emit('success', name + ' cookie set successfully');
};
...
module.exports.clearBrowserCookies = async () => {
    ...
    xhrEvent.emit('success', 'Browser cookies deleted successfully');
};
...
module.exports.deleteCookies = async (cookieName, options = {}) => {
    ...
    xhrEvent.emit('success', cookieName + ' cookie deleted successfully');
};
...
module.exports.setLocation = async (options) => {
    ...
    xhrEvent.emit('success', 'Geolocation set');
};
...
module.exports.goto = async (url, options = { navigationTimeout: config.navigationTimeout, waitForEvents: ['firstMeaningfulPaint'] }) => {
    ...
    xhrEvent.emit('success' ,'Navigated to url ' + url);
};
...
module.exports.reload = async (url, options = { navigationTimeout: config.navigationTimeout, waitForEvents: ['firstMeaningfulPaint'] }) => {
    ...
    xhrEvent.emit('success', url + 'reloaded');
};
...
module.exports.goBack = async (options = { navigationTimeout: config.navigationTimeout, waitForEvents: ['firstMeaningfulPaint'] }) => {
    validate();
    await _go(-1, options);
    xhrEvent.emit('success', 'Performed clicking on browser back button');
};
...
module.exports.goForward = async (options = { navigationTimeout: config.navigationTimeout, waitForEvents: ['firstMeaningfulPaint'] }) => {
    validate();
    await _go(+1, options);
    xhrEvent.emit('success', 'Performed clicking on browser forward button');
};
...
async function click(selector, options = {}, ...args) {
    ...
    if (isSelector(selector) || isString(selector) || Number.isInteger(selector)){
        ...
        xhrEvent.emit('success', 'Clicked ' + description(selector, true));
    } else {
        options = setOptions(options, selector.x, selector.y);
        await waitForMouseActions(options);
        xhrEvent.emit('success', 'Clicked on coordinates x : ' + selector.x + ' and y : ' + selector.y);
    }
}
...
module.exports.doubleClick = async (selector, options = {}, ...args) => {
    ...
    xhrEvent.emit('success', 'Double clicked ' + description(selector, true));
};
...
module.exports.rightClick = async (selector, options = {}, ...args) => {
    ...
    await click(selector, options, ...args);
    xhrEvent.emit('success', 'Right clicked ' + description(selector, true));
};
...
module.exports.hover = async (selector, options = {}) => {
    ...
    xhrEvent.emit('success', 'Hovered over the ' + description(selector, true));
};
...
module.exports.focus = async (selector, options = {}) => {
    ...
    xhrEvent.emit('success', 'Focussed on the ' + description(selector, true));
};
...
module.exports.attach = async (filepath, to) => {
    ...
    xhrEvent.emit('success', 'Attached ' + resolvedPath + ' to the ' + description(to, true));
};
...
async function _press(keys, options) {
    ...
    xhrEvent.emit('success', 'Pressed the ' + keys.reverse().join(' + ') + ' key');
}
...
async function highlight(selector, ...args) {
    ...
    xhrEvent.emit('success', 'Highlighted the ' + description(selector, true));
}
...
async function mouseAction(action, coordinates, options = {}) {
    ...
    xhrEvent.emit('success', 'Performed mouse ' + action + 'action at {' + coordinates.x + ', ' + coordinates.y + '}');
}
...
module.exports.scrollTo = async (selector, options = {}) => {
    ...
    if (headful) await highlightElemOnAction(await element(selector));
    xhrEvent.emit('success', 'Scrolled to the ' + description(selector, true));
};
...
const scroll = async (e, px, scrollPage, scrollElement, direction) => {
    ...
    xhrEvent.emit('success', 'Scrolled ' + direction + description(e, true) + 'by ' + px + ' pixels');
};
...
module.exports.screenshot = async (selector, options = {}) => {
    ...
    xhrEvent.emit('success', 'Screenshot is created at ' + options.path); 
};
...
module.exports.$ = (selector, ...args) => {
    ...
    return {
        get: getIfExists(get),
        exists: exists(getIfExists(get), xhrEvent),
...
module.exports.image = (attrValuePairs, ...args) => {
    ...
    return { get: getIfExists(get), exists: exists(getIfExists(get), xhrEvent), description: desc(selector, 'alt', 'Image'), text: selectorText(get) };
};
...
module.exports.link = (attrValuePairs, ...args) => {
    ...
    return { get: getIfExists(get), exists: exists(getIfExists(get), xhrEvent), description: desc(selector, 'text', 'Link'), text: selectorText(get) };
};
...
module.exports.listItem = (attrValuePairs, ...args) => {
    ...
    return { get: getIfExists(get), exists: exists(getIfExists(get), xhrEvent), description: desc(selector, 'label', 'List item'), text: selectorText(get) };
};
...
module.exports.button = (attrValuePairs, ...args) => {
    ...
    return { get: getIfExists(get), exists: exists(getIfExists(get), xhrEvent), description: desc(selector, 'label', 'Button'), text: selectorText(get) };
};
/**
...
module.exports.tap = async (selector, options = {}, ...args) => {
    ...
    xhrEvent.emit('success', 'Tap has been performed');
};
...
module.exports.comboBox = (attrValuePairs, ...args) => {
    ...
    return {
        ...
        select: async (value) => {
            ...
            xhrEvent.emit('success', 'Selected ' + value);
        },
        value: async () => {
            const nodeId = (await getIfExists(get)())[0];
            if (!nodeId) throw `${desc(selector, 'label', 'Combobox')} not found`;
    ...
    return {
        ...
        check: async () => {
            ...
            await doActionAwaitingNavigation(options, async () => {
                ...
                xhrEvent.emit('success', desc(selector, 'label', 'Checkbox') + 'is checked');
            });
        },
        uncheck: async () => {
            const options = setNavigationOptions({});
            await doActionAwaitingNavigation(options, async () => {
                ...
                xhrEvent.emit('success', desc(selector, 'label', 'Checkbox') + 'is unchecked');
            });
        },
        text: selectorText(get)
    };
    ...
    return {
        ...
        select: async () => {
            ...
            await doActionAwaitingNavigation(options, async () => {
                ...
                if (headful) await highlightElemOnAction(nodeId);
                await evaluate(nodeId, function getvalue() { this.checked = true; return true; });
                xhrEvent.emit('success', desc(selector, 'label', 'Radio Button') + 'is selected');
            });
        },
        deselect: async () => {
            const options = setNavigationOptions({});
            await doActionAwaitingNavigation(options, async () => {
                ...
                if (headful) await highlightElemOnAction(nodeId);
                await evaluate(nodeId, function getvalue() { this.checked = false; return true; });
                xhrEvent.emit('success', desc(selector, 'label', 'Radio Button') + 'is deselected');
            });
        },
        text: selectorText(get)
    };
...
module.exports.text = (text, ...args) => {
    ...
    return { get: getIfExists(get), exists: exists(getIfExists(get), xhrEvent), description: `Element with text "${text}"`, text: selectorText(get) };
};
...
function contains(text, ...args) {
    ...
    return { get: getIfExists(get), exists: exists(getIfExists(get), xhrEvent), description: `Element containing text "${text}"`, text: selectorText(get) };
}
...
function match(text, ...args) {
    ...
    return { get: getIfExists(get), exists: exists(getIfExists(get), xhrEvent), description: `Element matching text "${text}"`, text: selectorText(get) };
}
...
module.exports.evaluate = async (selector, callback, options = {}) => {
    ...
    xhrEvent.emit('success', 'Evaluated given script. Result:' + result.result.value);
...
module.exports.accept = async (text = '') => {
    ...
    xhrEvent.emit('success', 'Accepted dialog');
};
...
module.exports.dismiss = async () => {
    ...
    xhrEvent.emit('success', 'Dismissed dialog');
};
