...
let chromeProcess, temporaryUserDataDir, page, network, runtime, input, client, dom, emulation, overlay, criTarget, currentPort, currentHost, rootId = null,
    headful, security, ignoreSSLErrors, observe, observeTime, executionContextIdes;
        ...
        try {
            ...
            await cri({ target }, async (c) => {
                ...
                runtime.executionContextCreated(event => {
                    console.log(` executionContextCreated ===> : ${JSON.stringify(event)}`);
                    executionContextIdes.push(event.context.id);
                });
 ...
 * dragAndDrop($("work"),{top:10,bottom:10,left:10,right:10})
 * 
...
const $$ = async selector => {
    ...
    const getNodeIdes = async (executionContextId) => {
        let nodeIds = [];
    ...
    for (const contextId of executionContextIdes) {
        let nodeIds = await getNodeIdes(contextId);
        if (nodeIds.length)
