...
module.exports.goto = async (url, options = { timeout: 5000 }) => {
    ...
    await waitForNavigation(options.timeout, promises).catch((e) => {
        if(e === 'Timedout') throw new Error(`Couldn't load url ${url} in ${options.timeout}ms. Please increase the timeout.`);
    });
...
async function click(selector, options = {}, ...args) {
    ...
    if (!isElemAtPoint) isElemAtPoint = await checkIfChildOfOtherMatches(elems, parseInt(x), parseInt(y));
    if (!isElemAtPoint) throw Error(description(selector) + ' is coverred by other element');
    ...
    xhrEvent.addListener('xhrEvent', func);
    //TODO: Refactor to avoid creating too many listeners
    ...
    if (options.waitForNavigation) {
        await waitForNavigation(options.timeout, promises).catch((e) => {
            if(e === 'Timedout') throw new Error(`Navigation took more than ${options.timeout}ms. Please increase the timeout.`);
        });
    }
    xhrEvent.removeListener('xhrEvent', func);
...
const waitForPromises = (promises) => {
    return Promise.race([waitFor(200),new Promise(function waitForPromises(resolve) {
        if (promises.length) {
            resolve();
        }
        else setTimeout(() => { waitForPromises(resolve); }, 500);
    })]);
};

const waitForNavigation = (timeout, promises = []) => {
    promises.push(setRootId());
    //TODO: optimise wait for load, now even if there is no navigation it waits for load event and then timeout
