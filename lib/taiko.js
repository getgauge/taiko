...
module.exports.goto = async (url) => {
    ...
    await waitForNavigation(default_timeout);
 ...
 * @returns {Promise<Object>} - Object with the description of the action performed.
 */
module.exports.write = async (text, into, options = { delay: 100 }) => {
    validate();
    if (into) {
...
module.exports.toLeftOf = selector => {
    ...
    return new RelativeSearchElement(async (e, v) => {
        ...
        return rect.left < v; }, rectangle(selector, r => r.left), 
...
module.exports.toRightOf = selector => {
    ...
    return new RelativeSearchElement(async (e, v) => {
        const rect =  await boundingBox.getBoundingClientRect(dom,e);
        return rect.right > v; }, rectangle(selector, r => r.right), 
    isString(selector)?`To Right of ${selector}`:`To Right of ${selector.description}`);
};
...
module.exports.above = selector => {
    ...
    return new RelativeSearchElement(async (e, v) => {
        const rect =  await boundingBox.getBoundingClientRect(dom,e);
        return rect.top < v; }, rectangle(selector, r => r.top), 
    isString(selector)?`Above ${selector}`:`Above ${selector.description}`);
};
...
module.exports.below = selector => {
    ...
    return new RelativeSearchElement(async (e, v) => {
        const rect =  await boundingBox.getBoundingClientRect(dom,e);
        return rect.bottom > v;}, rectangle(selector, r => r.bottom), 
    isString(selector)?`Below ${selector}`:`Below ${selector.description}`);
};
...
module.exports.near = (selector) => {
    ...
    },rectangle(selector, r => r), 
    isString(selector)?`Near ${selector}`:`Near ${selector.description}`);
};
...
const setRootId = async() => {
    const {root: {nodeId}} = await dom.getDocument();
    return new Promise(resolve => rootId = nodeId);
};

const waitForNavigation = (timeout) =>{
    return Promise.race([setRootId(), waitFor(timeout)]);
};
