...
/**
 * Returns window's location attributes as requested.
 * @param {String} locationAttr - Provide the location attribute to fetch specific value.
 * @value {"protocol"} http, https
 * @value {"host"} abcd.com
 * @value {"pathname"} /mypage
 * @value {"search"} ?q=test
 * @value {"toString()"} http://abcd.com/mypage?q=test
 *
 * @example
 * browserURL('protocol');
 ...
 */
module.exports.browserURL = async (locationAttr = 'toString()') => {
    validate();
    if (['protocol', 'host', 'pathname', 'search', 'toString()'].includes(locationAttr)){
        return await _browserLocation(locationAttr);
    }
    else{
      throw Error('The param value for locationAttr does not fall within allowed values.\n Allowed values: protocol, host, pathname, search, toString() \n');
    }
};

const _browserLocation = async (locationAttr) => {
  let query = 'window.location.' + locationAttr;
  const locationObj = await runtimeHandler.runtimeEvaluate(query);
