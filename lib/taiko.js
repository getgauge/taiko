...
async function emulateDevice(deviceModel) {
    ...
    if (deviceEmulate == undefined)
        throw new Error(`Please set one of the given device models ${deviceNames.join('\n')}`);
    await Promise.all([
...
const _write = async (text, activeElement, options) => {
    if (activeElement.notWritable){
        await waitUntil(async () => !(await runtimeHandler.activeElement()).notWritable,1000,10000);
