 ...
 * await paste('admin')
 * @example
 * await paste('admin', into(textBox("Username"),{force:true})
 *
 * @param {string} text - Text to type into the element.
 * @param {selector|Element|string} into - A selector of an element to write into.
 * @param {Object} options
 ...
 * @returns {Promise<void>}
 */
 module.exports.paste = async (text, into, options = { delay: 0 }) => {
  validate();
  const { paste } = require('./actions/paste');
  options = setNavigationOptions(options);
  const desc = await paste(text, into, options);
  descEvent.emit('success', desc);
};
