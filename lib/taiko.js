...
module.exports.openBrowser = async options => {
    ...
    if(options && options.headless) args.push('--headless');
...
const getValues = (attribute,value,args) => {
    
    let label;

    if(attribute instanceof RelativeSearchElement){
        args = [attribute].concat(args);
        attribute = undefined;
    }    
    ...
    return {label:label,attribute:attribute,value:value,args:args};
};


const getElementGetter = (selector,query,tag) => {
    let get;
    if(selector.attribute && selector.value) get = async () => await handleRelativeSearch(await $$(`${tag}[${selector.attribute}="${selector.value}"]`), selector.args);
...
const desc = (selector,query,tag) => {
    let description = '';
    if(selector.attribute && selector.value) description = `${tag} with [${selector.attribute} = "${selector.value}"] `;
 ...
 * @returns {ElementWrapper}
 */
module.exports.image = (attribute,value, ...args) => {
    validate();
    const selector = getValues(attribute,value,args); 
    const get = getElementGetter(selector,async () => await $$xpath(`//img[contains(@alt, ${xpath(selector.label)})]`),'img');
    return { get: get, exists: exists(get), description: desc(selector,'alt','Image') };
};
 ...
 * @returns {ElementWrapper}
 */
module.exports.link = (attribute, value, ...args) => {
    validate();
    const selector = getValues(attribute,value,args);
    const get =  getElementGetter(selector,async () => await elements(selector.label, 'a'),'a');
    return { get: get, exists: exists(get), description: desc(selector,'text','Link')};
};
 ...
 * @returns {ElementWrapper}
 */
module.exports.listItem = (attribute,value, ...args) => {
    validate();
    const selector = getValues(attribute,value,args);
    const get =  getElementGetter(selector,async () => await elements(selector.label, 'li'),'li');
    return { get: get, exists: exists(get), description: desc(selector,'label','List item') };
};
 ...
 * @returns {ElementWrapper}
 */
module.exports.button = (attribute,value, ...args) => {
    validate();
    const selector = getValues(attribute,value,args);
    const get =  getElementGetter(selector,async () => await elements(selector.label, 'button'),'button');
    return { get: get, exists: exists(get), description: desc(selector,'label','Button') };
};
 ...
 * @returns {ElementWrapper}
 */
module.exports.inputField = (attribute, value, ...args) => {
    validate();
    const selector = getValues(attribute,value,args);
    const get =  getElementGetter(selector,async () => await $$xpath(`//input[@id=(//label[contains(text(), ${xpath(selector.label)})]/@for)]`),'input');
    return {
        get: get,
        exists: exists(get),
...
/**
 * This {@link selector} lets you identify a file input field on a web page either with label or with attribute and value pair.
 *
 * @example
 ...
 * @param {string} [attribute='value'] - The file field's attribute.
 * @param {string} value - Value of the attribute specified in the first parameter. 
 * @param {...relativeSelector} args
 * @returns {ElementWrapper}
 */
module.exports.fileField = fileField;
function fileField(attribute,value, ...args) {
    validate();
    let label;
    if (value instanceof RelativeSearchElement) {
        args = [value].concat(args);
    ...
    if(!value)label = attribute;
    const get = label ? async () => await handleRelativeSearch(await $$xpath(`//input[@type='file'][@id=(//label[contains(text(), ${xpath(label)})]/@for)]`), args) :
        async () => await handleRelativeSearch(await $$(`input[${attribute}="${value}"]`), args);  
    return {
        get: get,
        exists: exists(get),
        value:  async () => (await evaluate((await get())[0],function getvalue(){ return this.value;})).value,
        description: label ? `File input field with label containing "${label}"` : `File input field with attribute "${attribute} and "${value}"`,
    };
}

/**
 * This {@link selector} lets you identify a text field on a web page either with label or with attribute and value pair.
 *
 * @example
 * focus(textField('Username:'))
 * textField('Username:').exists()
 *
 * @param {string} [attribute='value'] - The text field's attribute.
 * @param {string} value - Value of the attribute specified in the first parameter. 
...
function textField(attribute,value, ...args) {
    validate();
    let label;
    if (value instanceof RelativeSearchElement) {
        args = [value].concat(args);
    ...
    if(!value)label = attribute;
    const get = label ? async () => await handleRelativeSearch(await $$xpath(`//input[@type='text'][@id=(//label[contains(text(), ${xpath(label)})]/@for)]`), args):
        async () => await handleRelativeSearch(await $$(`input[type="text"][${attribute}="${value}"]`), args);
    return {
        get: get,
        exists: exists(get),
        description: label ? `Text field with label containing "${label}"` : `Text field with attribute "${attribute} and "${value}"`,
        value:  async () => (await evaluate((await get())[0],function getvalue(){ return this.value;})).value,
    };
}

/**
 * This {@link selector} lets you identify a combo box on a web page either with label or with attribute and value pair.
 *
 * @example
 ...
 *
 * @param {string} [attribute='value'] - The comboBox's attribute.
 * @param {string} value - Value of the attribute specified in the first parameter. 
 * @param {string} label - The label (human-visible name) of the combo box.
 * @param {...relativeSelector} args
 * @returns {ElementWrapper}
 */
module.exports.comboBox = (attribute,value, ...args) => {
    validate();
    let label;
    if (value instanceof RelativeSearchElement) {
        args = [value].concat(args);
    ...
    if(!value)label = attribute;
    const get = label ? async () => await handleRelativeSearch(await $$xpath(`//select[@id=(//label[contains(text(), ${xpath(label)})]/@for)]`), args)
        : async () => await handleRelativeSearch(await $$(`input[type="combobox"][${attribute}="${value}"]`), args);
    return {
        get: get,
        exists: exists(get),
        description: label ? `Combobox with label containing "${label}"` : `Combobox with attribute "${attribute} and "${value}"`,
        select: async (value) => {
            ...
            function selectBox(value) {
                Array.from(this.options).filter(o => o.text === value).forEach(o => o.selected = true);
                return true;
            }
            const { object: { objectId } } = await dom.resolveNode({ nodeId });
            await runtime.callFunctionOn({
...
/**
 * This {@link selector} lets you identify a checkbox on a web page either with label or with attribute and value pair.
 *
 * @example
 ...
 *
 * @param {string} [attribute='value'] - The checkBox's attribute.
 * @param {string} value - Value of the attribute specified in the first parameter. 
 * @param {string} label - The label (human-visible name) of the check box.
 * @param {...relativeSelector} args
 * @returns {ElementWrapper}
 */
module.exports.checkBox = (attribute,value, ...args) => {
    validate();
    let label;
    if (value instanceof RelativeSearchElement) {
        args = [value].concat(args);
    ...
    if(!value)label = attribute;
    const get = label ? async () => await handleRelativeSearch(await $$xpath(`//input[@type='checkbox'][@id=(//label[contains(text(), ${xpath(label)})]/@for)]`), args):
        async () => await handleRelativeSearch(await $$(`input[type="checkbox"][${attribute}="${value}"]`), args);
    return {
        get: get,
        exists: exists(get),
        description: label ? `Checkbox with label containing "${label}"` : `Checkbox with attribute "${attribute} and "${value}"`,
...
/**
 * This {@link selector} lets you identify a radio button on a web page either with label or with attribute and value pair.
 *
 * @example
 ...
 *
 * @param {string} [attribute='value'] - The radioButton's attribute.
 * @param {string} value - Value of the attribute specified in the first parameter. 
 * @param {string} label - The label (human-visible name) of the radio button.
 * @param {...relativeSelector} args
 * @returns {ElementWrapper}
 */
module.exports.radioButton = (attribute,value, ...args) => {
    validate();
    let label;
    if (value instanceof RelativeSearchElement) {
        args = [value].concat(args);
    ...
    if(!value)label = attribute;
    const get = label ? async () => await handleRelativeSearch(await $$xpath(`//input[@type='radio'][@id=(//label[contains(text(), ${xpath(label)})]/@for)]`), args):
        async () => await handleRelativeSearch(await $$(`input[type="radio"][${attribute}="${value}"]`), args);
    return {
        get: get,
        exists: exists(get),
        description: label ? `Radio button with label containing "${label}"` : `Radio button with attribute "${attribute} and "${value}"`,
...
module.exports.toLeftOf = selector => {
    ...
    return new RelativeSearchElement(async (e, v) => {
        ...
        return rect.left < v; }, rectangle(selector, r => r.left), `To left of ${selector}`);
};
...
module.exports.toRightOf = selector => {
    ...
    return new RelativeSearchElement(async (e, v) => {
        const rect =  await boundingBox.getBoundingClientRect(dom,e);
        return rect.right > v; }, rectangle(selector, r => r.right), `To right of ${selector}`);
};
...
module.exports.above = selector => {
    ...
    return new RelativeSearchElement(async (e, v) => {
        const rect =  await boundingBox.getBoundingClientRect(dom,e);
        return rect.top < v; }, rectangle(selector, r => r.top), `Above ${selector}`);
};
...
module.exports.below = selector => {
    ...
    return new RelativeSearchElement(async (e, v) => {
        const rect =  await boundingBox.getBoundingClientRect(dom,e);
        return rect.bottom > v;}, rectangle(selector, r => r.bottom), `Below ${selector}`);
};
...
const elements = async (selector, options, tag) => {
...
class RelativeSearchElement {
    ...
    async isValid(nodeId) {
        let valid = true;
        const rect_values = await this.value;
        for(const rect_value of rect_values){
            valid = await this.condition(nodeId, rect_value);
        }
        return valid;
    }
