 ...
 * 
 * @param {object} matcher - values to match a request(url, post-data, method, resourceType, headers) any of these can be used. eg. {url:"https://taiko.gauge.org/assets/css/pilcrow.css",method:'GET'}
 * @param {object} action - values to override request (url, method, post-data, headers) or mock-response (errorCode, status, headers, contentType, body)
 */
module.exports.intercept = async (requestMatcher,action) => {
    networkHandler.addInterceptor(requestMatcher,action);
    return {description: `Interceptor added for ${requestMatcher.url}`};
};
