...
async function _click(selector, options, ...args) {
  if (isSelector(selector) || isString(selector)) {
    const isDisabledFn =
      selector.isDisabled || match(selector).isDisabled;
    if (isDisabledFn && (await isDisabledFn())) {
...
module.exports.image = (attrValuePairs, ...args) => {
  ...
  return {
    get: getIfExists(get),
    exists: exists(getIfExists(get), descEvent),
    isDisabled: isDisabled(getIfExists(get), descEvent),
...
const _focus = async selector => {
  ...
  if (isSelector(selector) ) {
    const isDisabledFn = selector.isDisabled;
    if (isDisabledFn && (await isDisabledFn())) {
      throw Error(description(selector) + ' is disabled');
    }
  ...
  function focusElement() {
    if (this.disabled == true)
      throw Error('Element is not focusable');
    this.focus();
    return true;
  }
};
