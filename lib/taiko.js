...
const { helper, assert, waitFor, isString } = require('./helper');
...
let chromeProcess, page, network, runtime, input, client, dom, emulation, criTarget, currentPort, currentHost, rootId = null;
 ...
 *
 * @param {Object} options {headless: true|false, args:['--window-size=1440,900]} Args to open chromium https://peter.sh/experiments/chromium-command-line-switches/.
...
module.exports.openBrowser = async (options = { headless: true }) => {
    ...
    let args = [`--remote-debugging-port=${options.port}`];
    if (!args.some(arg => arg.startsWith('--user-data-dir'))) {
        let temporaryUserDataDir = await mkdtempAsync(CHROME_PROFILE_PATH);
 ...
 * @returns {Promise<Object>} - Object with the description of the action performed.
 */
module.exports.openTab = async (targetUrl) => {
    const target = await criTarget.createTarget({ url: targetUrl });
    await connect_to_cri(targetHandler.constructCriTarget(target));
    const { root: { nodeId } } = await dom.getDocument();
    rootId = nodeId;
...
module.exports.closeTab = async (targetUrl) => {
    ...
    await criTarget.closeTarget({ targetId: target.id });
...
async function click(selector, options = {}, ...args) {
    ...
    for (let elem of elems) {
        ...
        if (isElemAtPoint) {
            await highlight(selector, { scrollTo: false });
            break;
        }
    }
    ...
    await _removeHighlight(selector);
...
async function highlight(selector, options = { scrollTo: true }) {
    validate();
    if (options.scrollTo) await scrollTo(selector);
...
async function _removeHighlight(selector) {
    validate();
    function removeHighlightOnNode() {
        this.style.border = 'none';
        return true;
    }
    await evaluate(selector, removeHighlightOnNode);
    return { description: 'Removed highlight for ' + description(selector, true) };
}
 ...
 */
module.exports.intercept = async (requestUrl, action) => {
    await networkHandler.addInterceptor({ requestUrl: requestUrl, action: action });
...
const waitForPromises = (promises, waitForStart) => {
    return Promise.race([waitFor(waitForStart), new Promise(function waitForPromise(resolve) {
        ...
        else setTimeout(() => { waitForPromise(resolve); }, waitForStart / 5);
    })]);
};
...
module.exports.metadata = {
    ...
    'Helpers': ['intervalSecs', 'timeoutSecs', 'to', 'into', 'waitFor', 'accept', 'dismiss']
};