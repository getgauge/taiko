...
async function click(selector, options = {}, ...args) {
    ...
    const e = (await handleRelativeSearch(await elements(selector), args))[0];
    await scrollTo(e);
    const type = (await evaluate(e, function getType() { return this.type; })).value;
...
function match(text, ...args) {
    ...
    const get = async (e = '*') => {
        ...
        if (!elements || !elements.length) elements = await $$xpath('//' + e + `[translate(normalize-space(.),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')=${xpath(text.toLowerCase())}]`);
        return await handleRelativeSearch(elements, args);
    };
 ...
 * 
 * @example
 * intercept(request({url:"https://taiko.gauge.org/assets/css/pilcrow.css",method:'GET'}),override({errorCode:'Aborted'}))
 * 
 * @param {object} request
 * @param {object} action
 */
module.exports.intercept = async (request,action) => {
    const options = {requestIdentifier:request};
    if(action.override) options.override = action.override;
    else options.mockResponse = action.mockResponse;
    networkHandler.addInterceptor(options);
    return {description: `Interceptor added for ${request.url}`};
};

/**
 * Gives back request objects improves readability
 * 
 * @param {object} requestIdentifier - values to match a request(url, post-data, method, resourceType, headers) any of these can be used. eg. {url:"https://taiko.gauge.org/assets/css/pilcrow.css",method:'GET'}
 */
module.exports.request = (requestIdentifier) => requestIdentifier;

/**
 * Gives back request override objects improves readability
 * 
 * @param {object} override - values to override request (url, method, post-data, headers, errorReason)
 */
...
/**
 * Gives back mock response objects improves readability
 * 
 * @param {object} mockResponse - values to mock response (status, header, content-type, body)
 */
...
const waitForNavigation = (timeout, promises = []) => {
    if (promises.length === 0) {
        let firstXhrEventPromise = new Promise((resolve) => {
            xhrEvent.once('xhrEvent', () => {
                setTimeout(() => { resolve(); }, 500);
            });
        });
        promises.push(firstXhrEventPromise);
    }
    promises.push(setRootId());
    return Promise.race([waitFor(timeout), Promise.all(promises)]);
};
...
const $$ = async selector => {
    if (rootId == null) await setRootId();
...
const handleRelativeSearch = async (elements, args) => {
    ...
    const matchingNode = await getMatchingNode(elements, args);
    return matchingNode === undefined ? [] : [matchingNode];
};
...
module.exports.metadata = {
    ...
    'Helpers': ['intervalSecs', 'timeoutSecs', 'to', 'into', 'waitFor', 'accept', 'dismiss', 'request', 'override', 'mockResponse']
};