...
const getMatchingNodes = async(elements,args) => {
    ...
    for(const element of elements){
        ...
        for(const arg of args){
            const matchingNode = await arg.validNodes(element);
        ...
        if(valid) filteredElementsMatchingNodes.push({element:element,matchedNodes:matchedNodes});
    }
    return filteredElementsMatchingNodes;
};

const findDistance = async (elem) => {
    let distance = 0;
    for(const node of elem.matchedNodes) distance += await boundingBox.getPositionalDifference(dom,elem.element,node);
    return distance;
};
...
const handleRelativeSearch = async (elements, args) => {
    ...
    let mostRelativeElem=[], minDistance = Infinity;
    for(const elem of filteredElementsMatchingNodes){
        const elemDist = await findDistance(elem);
        if(elemDist < minDistance){
            minDistance = elemDist;
            mostRelativeElem = [elem.element];
        } 
    }
    return mostRelativeElem;
};
