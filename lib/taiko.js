...
let chromeProcess, temporaryUserDataDir, page, network, runtime, input, client, dom, emulation, overlay, criTarget, currentPort, currentHost,
    headful, security, ignoreSSLErrors, observe, observeTime, browser, tracing, io;
    ...
    return new Promise(async function connect(resolve) {
        try {
            ...
            await cri({ target }, async (c) => {
                ...
                tracing = new TracingHandler(tracing,io);
...
/**
 * Returns the Speed Index and Perceptual Speed Index 
 * from the page load that happened between the tracing period.
 * https://developers.google.com/web/tools/lighthouse/audits/speed-index
 * @example
 * getSpeedIndex()
 * 
 * @returns {Promise<Object>} - Object with speedIndex and perceptualSpeedIndex
 *
 */
module.exports.getSpeedIndex = async () => {
    return await tracing.getSpeedIndex();
};

/**
 * Returns the tracelogs that was captured within the tracing period. 
 * You can use this command to store the trace logs on the file system 
 * to analyse the trace via Chrome DevTools interface.
 * 
 * @example
 * fs.writeFileSync('tracelog.json', JSON.stringify(await getTracingLogs()));
 * 
 * @returns {Promise<Object>} - Object with tracing logs
 */
module.exports.getTracingLogs = async () => {
    return await tracing.getTracingLogs();
};

module.exports.metadata = {
    'Browser actions': ['openBrowser', 'closeBrowser', 'client', 'switchTo', 'setViewPort', 'openTab', 'closeTab', 'overridePermissions', 'clearPermissionOverrides', 'setCookie', 'clearBrowserCookies', 'deleteCookies', 'setLocation','startTracing','endTracing','getSpeedIndex','getTracingLogs'],
