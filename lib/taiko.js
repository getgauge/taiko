...
const findMinDifference = async (elem) => {
    const matchingNodesCartesian = cartesian(elem.matchingNodes);
    let minDiff = Infinity;
    for(const combination of matchingNodesCartesian){
        let diff = 0;
        for(const node of combination){
            diff += await boundingBox.getPositionalDifference(dom,elem.element,node);
        }
...
const handleRelativeSearch = async (elements, args) => {
    ...
    let mostRelativeElem=[], minDiff = Infinity;
    for(const elem of filteredElementsMatchingNodes){
        const elemMinDiff = await findMinDifference(elem);
        if(elemMinDiff < minDiff){
            minDiff = elemMinDiff;
