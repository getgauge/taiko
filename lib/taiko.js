...
module.exports.near = (selector) => {
    ...
    const nearOffset = 30;
    const isWithinGivenOffSet = (offsetDelta) => offsetDelta < nearOffset;
    const offSetDelta = (firstOffSet, secondOffSet) => Math.abs( firstOffSet, secondOffSet);
    return new RelativeSearchElement(async (e, v) => {
        const rect = await domHandler.getBoundingClientRect(e);
        return ( isWithinGivenOffSet(offSetDelta(rect.bottom - v.top)) ||isWithinGivenOffSet(offSetDelta(rect.top - v.bottom))) &&
        ( isWithinGivenOffSet(offSetDelta(rect.left - v.right)) || isWithinGivenOffSet(offSetDelta(rect.right - v.left)) );
