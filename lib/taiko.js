...
const _go = async (delta, options = {timeout: 30000}) =>{
  ...
  const [response] = await Promise.all([
    await waitForNavigation(options.timeout).catch(handleTimeout(options.timeout)),
...
module.exports.metadata = {
    'Browser actions': ['openBrowser', 'closeBrowser', 'client', 'switchTo', 'setViewPort', 'openTab', 'closeTab', 'goBack', 'goForward'],
