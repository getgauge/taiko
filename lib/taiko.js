...
const checkIfElementAtPointOrChild = async (e, x, y) => {
    ...
    function isElementAtPointOrChild(value) {
        ...
        return this.contains(node) || (window.getComputedStyle(node).getPropertyValue('opacity') < 0.1);
    }
...
module.exports.write = async (text, into, options = { delay: 10 }) => {
    ...
    await doActionAwaitingNavigation(options, async () => {
        if (into) {
            ...
            description = `Wrote ${text} into the ` + description(selector, true);
            return;
        } else {
            if (await _getActiveElementTag() !== 'BODY') {
                await _write(text, options);
                description = `Wrote ${text} into the focused element.`;
                return;
            }
            throw new Error('Not focused into a writable field. Please use "write(\'admin\', into(\'Username:\'))" or increase the delay.');
        }
    });
