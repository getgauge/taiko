...
const setNavigationPromises = (options) => {
    const promises = [];
    ...
    const customEventsToWait = options.waitForEvents;
    options.waitForEvents = options.waitForEvents || ['firstMeaningfulPaint'];
    options.waitForEvents.forEach((event) => {
        promises.push(new Promise((resolve) => {
            xhrEvent.addListener(event, resolve);
        }));
    });
    if(!customEventsToWait){
 ...
 * @returns {Promise<Object>} - Object with the description of the action performed and the final URL.
 */
module.exports.reload = async (url, options = { timeout: default_timeout } ) => {
    validate();
    const promises = setNavigationPromises(options);
...
module.exports.goBack = async (options = {timeout: default_timeout}) =>{
    validate();
    await _go(-1, options);
...
module.exports.goForward = async (options = {timeout: default_timeout}) =>{
    validate();
    await _go(+1, options);
...
module.exports.inputField = (attrValuePairs, ...args) => {
    ...
    const selector = getValues(attrValuePairs, args);
    let get = async() => {
        let elements = await $$xpath(`//input[@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)]`);
        if(elements && elements.length)
            return getElementGetter(selector,
                async () => await $$xpath(`//input[@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)]`),
                'input')();
        return getElementGetter(selector,
            async () => await $$xpath(`//label[contains(string(), ${xpath(selector.label)})]/input`),
            'input')();
    };
    return {
        get: getIfExists(get),
...
module.exports.tap = async (selector, options = {}, ...args) => {
    ...
    if (!isElemAtPoint)
        throw Error(description(selector) + ' is coverred by other element');
    return { description: 'Tap has been performed'};
};
...
module.exports.comboBox = (attrValuePairs, ...args) => {
    ...
    const selector = getValues(attrValuePairs, args);
    let get = async() => {
        let elements = await $$xpath(`//select[@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)]`);
        if(elements && elements.length)
            return getElementGetter(selector,
                async () => await $$xpath(`select[@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)]`),
                'select')();
        return getElementGetter(selector,
            async () => await $$xpath(`//label[contains(string(), ${xpath(selector.label)})]/select`),
            'select')();
    };
    return {
        get: getIfExists(get),
...
module.exports.checkBox = (attrValuePairs, ...args) => {
    ...
    const selector = getValues(attrValuePairs, args);
    let get = async() => {
        let elements = await $$xpath(`//input[@type='checkbox'][@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)]`);
        if(elements && elements.length)
            return getElementGetter(selector,
                async () => await $$xpath(`//input[@type='checkbox'][@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)]`),
                'input[type="checkbox"]')();
        return getElementGetter(selector,
            async () => await $$xpath(`//label[contains(string(), ${xpath(selector.label)})]/input[@type='checkbox']`),
            'input[type="checkbox"]')();
    };
    return {
        get: getIfExists(get),
...
module.exports.radioButton = (attrValuePairs, ...args) => {
    ...
    const selector = getValues(attrValuePairs, args);
    let get = async() => {
        let elements = await $$xpath(`//input[@type='radio'][@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)]`);
        if(elements && elements.length)
            return getElementGetter(selector,
                async () => await $$xpath(`//input[@type='radio'][@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)]`),
                'input[type="radio"]')();
        return getElementGetter(selector,
            async () => await $$xpath(`//label[contains(string(), ${xpath(selector.label)})]/input[@type='radio']`),
            'input[type="radio"]')();
    };
    return {
        get: getIfExists(get),
...
module.exports.metadata = {
    ...
    'Page actions': ['goto', 'reload', 'goBack', 'goForward', 'currentURL', 'title', 'click', 'doubleClick', 'rightClick', 'dragAndDrop', 'hover', 'focus', 'write', 'clear', 'attach', 'press', 'highlight', 'scrollTo', 'scrollRight', 'scrollLeft', 'scrollUp', 'scrollDown', 'screenshot', 'tap'],
