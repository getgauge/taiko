...
/**
 * Sets local storage values to current session.
 * @example
 * openBrowser()
 * addToLocalStorage({key:"auth-token", value:"WERTYUIOJHGFDXCVBN!@#$%^&^%"})
 *
 * @param {Object} options {key, value} -- Key-Value pair to be set to the local storage
 * @returns {Promise<Object>} - Object with the description of the action performed.
 */
module.exports.addToLocalStorage = async (options) => {
  return await page.addScriptToEvaluateOnNewDocument({source: 'localStorage.setItem("' + options.key + '","' + options.value + '")'});
};
...
module.exports.metadata = {
    'Browser actions': ['openBrowser', 'closeBrowser', 'client', 'switchTo', 'emulateDevice', 'setViewPort', 'openTab', 'closeTab', 'overridePermissions', 'clearPermissionOverrides', 'setCookie', 'clearBrowserCookies', 'deleteCookies', 'setLocation', 'addToLocalStorage'],
