 ...
 * @param {boolean} [options.exactMatch=true] - `true`, or `false`.
...
async function click(selector, options={}) {
    validate();
    options = {
        exactMatch : options.exactMatch !== undefined ? options.exactMatch : true,
...
module.exports.attach = async (filepath, to, options = {}) => {
    ...
    const nodeId = await element(to, options);
    ...
    const elements = await (() => {
        if (isString(selector)) {
            if (options.exactMatch){ 
                ...
                return contains(selector).get(tag);
            }
        }
