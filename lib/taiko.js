...
module.exports.evaluate = async (selector, callback, options = {}) => {
    return {description:'Evaluated given script on ' + description(selector,true), result: evaluateCallback(selector, callback, options)};
};

const evaluateCallback = (selector, callback, options) => {
    return async () => {
        let result;
        ...
        const nodeId = await element(selector);
        if (headful) await highlightElemOnAction(nodeId);

        async function evalFunc(callback) {
            let fn;
            eval(`fn = ${callback}`);
            console.log(await fn(this));
