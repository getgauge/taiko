...
/**
 * This will return a new RegExp Object
 *
 * @returns {Object}
 */
module.exports.regex = async regexString => {
  var isValid = true;
  var myRegexObject = null;
  try {
    myRegexObject = new RegExp(regexString);
  } catch (e) {
    isValid = false;
  }
  if (!isValid) {
    throw new TypeError(`The "regexString" must be a valid regex`);
  } else {
    return myRegexObject;
  }
};
 ...
 * await swithTo(regex(/http(s?):\/\/(www?).google.(com|co.in|co.uk)/))
 * # switch using regex Title
 * await swithTo(regex(/Go*gle/))
 *
...
module.exports.switchTo = async targetUrl => {
  ...
  let q = targetUrl instanceof RegExp;
  const isRegexText = q ? ' Regex' : '';
  if (t !== 'string' && !q) {
    throw new TypeError(
  ...
  if (targets.matching.length === 0) {
    throw new Error(
      `No target with given ${isRegexText} URL/Title found.`,
    );
  }
  ...
  descEvent.emit(
    'success',
    `Switched to tab with${isRegexText} URL ` + targetUrl,
  );
};
 ...
 * await closeTab(regex(/Go*gle/))
 * # Closes all the tabs with Regex URL '/http(s?):\/\/(www?).google.(com|co.in|co.uk)/'
 * await closeTab(regex(/http(s?):\/\/(www?).google.(com|co.in|co.uk)/))
 *
...
module.exports.closeTab = async targetUrl => {
  ...
  if (!matching.length) {
    throw new Error('No target with given URL/Title/Regex found.');
  }
  let q = targetUrl instanceof RegExp;
  const isRegexText = q ? ' Regex' : '';
  ...
  let message = targetUrl
    ? `Closed all tabs with${isRegexText} URL ${targetUrl}`
    : `Closed current tab with${isRegexText} URL ${closedTabUrl}`;
...
/**
 * This {@link selector} lets you identify a text field(input (with type text, password, url, search, number, email, tel, textarea, contenteditable and range fields)
 ...
 * await textBox({min:'0',max:'100'}).value()
 *
 * @param {Object} attrValuePairs - Pairs of attribute and value like {"id":"name","class":"class-name"}
...
function textBox(attrValuePairs, ...args) {
  ...
  const inputTypesExps = [
    'input[@type="text" or @type="password" or @type="search" or @type="number" or @type="email" or @type="tel" or @type="url" or @type="range"]',
    'input[not(@type)]',
  ];
  if (!selector.attrValuePairs && !selector.label)
    get = async () =>
