...
module.exports.switchTo = async arg => {
  ...
  if (isObject(arg)) {
    ...
    if (contexts.size !== 0) {
      ...
      descEvent.emit('success', `Switched to Incognito window matching ${arg}`);
    }
  } else {
 ...
 * @example
 * await openWindow('https://google.com', { name: 'windowName', incognito: true }) - Opens a Incognito window
 * await openWindow('https://google.com', { name: 'windowName' }) - Opens normal window
 ...
 * @returns {Promise}
 */
module.exports.openWindow = async (
  url,
  options = { navigationTimeout: defaultConfig.navigationTimeout },
) => {
  validate();
  if (url === undefined || url === '') {
    throw new Error('Url needs to be provided to openWindow');
  }
  ...
  if (!options.name) {
    options.name = generateRandomString();
  }
  browser = await connect_to_cri(browserDebugUrl, { window: true });
  const targetId = await createBrowserContext(browser, url, runtimeHandler, options);
...
module.exports.closeWindow = async arg => {
  await closeBrowserContext(arg);
  _client = null;
  descEvent.emit('success', `Window with name ${arg} closed`);
};

const _switchToIncognitoBrowser = async arg => {
  await switchBrowserContext(connect_to_cri, arg);
  await dom.getDocument();
  descEvent.emit('success', `Switched to Incognito window ${arg.name}`);
};
...
module.exports.focus = async (selector, options = {}) => {
  ...
  await doActionAwaitingNavigation(options, async () => {
    if (defaultConfig.headful) {
      await highlightElemOnAction((await findFirstElement(selector)).get());
    }
...
function fileField(attrValuePairs, _options = {}, ...args) {
  ...
  const basicElementWrapper = generateElementWrapper(get, description, {
    value: value,
  });
...
function tableCell(options = {}, attrValuePairs, ...args) {
  ...
  var tempDescription;
  if (selector.label) {
    tempDescription = `tableCell at row:${options.row} and column:${options.col}`;
  } else {
    tempDescription = desc(selector, 'Table', 'Table Cell');
  }
...
function dropDown(attrValuePairs, ...args) {
  ...
  const select = async function(value) {
    ...
    const options = setNavigationOptions({});
    await doActionAwaitingNavigation(options, async () => {
      const result = await runtimeHandler.runtimeCallFunctionOn(selectBox, null, {
        nodeId: nodeId,
        arg: value,
        returnByValue: true,
      });
...
module.exports.checkBox = (attrValuePairs, _options, ...args) => {
  ...
  const isChecked = async function() {
    ...
    var description = val ? this.description + 'is checked' : this.description + 'is not checked';
    descEvent.emit('success', description);
    return val;
  };
