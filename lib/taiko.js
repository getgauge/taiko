...
eventHandler.addListener('targetCreated', async newTarget => {
  ...
  if (response) {
    ...
    await connect_to_cri(pages[0]).then(() => {
      logEvent(
        `Target Navigated: Target id: ${newTarget.targetInfo.targetId}`,
      );
...
module.exports.openBrowser = async (
  ...
  if (!options.profile && chromeProcess && !chromeProcess.killed) {
    throw new Error(
  ...
  if (options.profile) {
    await _openIncognitoBrowser(options.profile);
  } else {
    browser = await connect_to_cri();
  }
 ...
 * @returns {Promise}
 */
module.exports.closeBrowser = async (options = {}) => {
  validate();
  if (options.profile) {
    await _closeIncognitoBrowser(options.profile);
  } else {
    await _closeBrowser();
  }
...
const _closeBrowser = async () => {
  ...
  await chromeProcess.kill('SIGTERM');
...
module.exports.switchTo = async arg => {
  ...
  if (isObject(arg)) {
    if (arg.profile) {
      await _switchToIncognitoBrowser(arg.profile);
    }
  } else {
    if (
      typeof arg != 'string' &&
...
const _openIncognitoBrowser = async profileName => {
  if (!browserMode) {
    throw new Error(
      'Failed to open Incognito window in headfull mode',
    );
  }
  browser = await connect_to_cri(browserDebugUrl, { profiles: true });
  const targetId = await createBrowserContext(browser, profileName);
  await connect_to_cri(targetId);
  descEvent.emit(
    'success',
    `Incognito Browser opened with profile ${profileName}`,
  );
};

const _closeIncognitoBrowser = async profileName => {
  await closeBrowserContext(profileName);
  _client = null;
  descEvent.emit(
    'success',
    `Incognito ${profileName} Browser closed`,
  );
};

const _switchToIncognitoBrowser = async profileName => {
  await switchBrowserContext(connect_to_cri, profileName);
  await dom.getDocument();
  descEvent.emit(
    'success',
    `Switched to Incognito profile ${profileName}`,
  );
};
