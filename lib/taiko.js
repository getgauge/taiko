...
const { helper, waitFor, isString, isFunction,commandlineArgs } = require('./helper');
...
module.exports.openBrowser = async (options = { headless: true }) => {
    ...
    const device = process.env.TAIKO_EMULATE_DEVICE || commandlineArgs().emulateDevice;
    if (device) this.emulateDevice(device);
...
module.exports.emulateDevice = async (deviceModel) => {
    const deviceEmulate = device[deviceModel];
    let devices = Object.keys(device);
    if (deviceEmulate == undefined) 
        throw new Error(`Please set one of the given device model ${devices.join('\n')}`);
    await Promise.all([
...
module.exports.metadata = {
    ...
    'Page actions': ['goto', 'reload', 'goBack', 'goForward', 'title', 'click', 'doubleClick', 'rightClick', 'dragAndDrop', 'hover', 'focus', 'write', 'clear', 'attach', 'press', 'highlight', 'scrollTo', 'scrollRight', 'scrollLeft', 'scrollUp', 'scrollDown', 'screenshot', 'currentURL','emulateDevice'],
