...
module.exports.openBrowser = async (options = { headless: true }) => {
    ...
    plugins.forEach((p) => p.init(module.exports));
...
module.exports.inputField = (attrValuePairs, ...args) => {
    ...
    const get = getElementGetter(selector, async () =>
        await $$xpath(`//input[@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)]`), 'input');
    return {
        get: getIfExists(get),
        exists: exists(getIfExists(get)),
...
/* TODO:
    * destructuring import support -> Done
    * provide taiko api inside plugin -> Done
    * validate plugins -> not required, plugin is loaded at user side

    * Add js docs for loadPlugin
    * Create a plugin generator
    * Autocompletion for plugin apis
    * repl apis extensibility support for plugins
*/

module.exports.loadPlugin = (plugin) => {
    try {
        if (!plugins.has(plugin.id())) {
            if (client !== undefined) plugin.init(module.exports);
