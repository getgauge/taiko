...
const { setConfig, config, setNavigationOptions, setClickOptions, setBrowserOptions } = require('./config');
const { filterVisibleNodes } = require('./nodeFilters');
const fs = require('fs');
const os = require('os');
...
const xhrEvent = new EventEmitter();
const descEvent = new EventEmitter();
let chromeProcess, temporaryUserDataDir, page, network, runtime, input, _client, dom, emulation, overlay, criTarget, 
    currentPort, currentHost, security, browser, device;
let xhrEventProxy;
let clientProxy;
    ...
    return new Promise(async function connect(resolve) {
        try {
            ...
            await cri({ target, local: localProtocol }, async (c) => {
                ...
                if (config.ignoreSSLErrors) security.setIgnoreCertificateErrors({ ignore: true });
...
async function _click(selector, options, ...args) {
    ...
    for (let elem of elems) {
        ...
        if (isElemAtPoint) {
            ...
            if (config.headful) await highlightElemOnAction(elem);
            break;
        }
    }
...
module.exports.hover = async (selector, options = {}) => {
    ...
    await scrollTo(e);
    if (config.headful) await highlightElemOnAction(e);
...
module.exports.focus = async (selector, options = {}) => {
    ...
    options = setNavigationOptions(options);
    await doActionAwaitingNavigation(options, async () => {
        if (config.headful) await highlightElemOnAction(await element(selector));
...
const _write = async (text, activeElement, options) => {
    ...
    if (config.headful) await highlightElemOnAction(activeElement.nodeId);
...
module.exports.attach = async (filepath, to) => {
    ...
    if (config.headful) await highlightElemOnAction(nodeId);
...
const prepareParameters = (attrValuePairs, options, ...args) => {
    ...
    let values = { selector:  { attrValuePairs: attrValuePairs, args: args } };
...
const getElementGetter = (selector, query, tags, selectHiddenElement) => {
    ...
    if (selector.attrValuePairs) {
        ...
        get = async () => await handleRelativeSearch(await $$xpath(query, selectHiddenElement), selector.args);
    }
    else if (selector.label) get = async () => await handleRelativeSearch(await query(), selector.args);
    else get = async () => await handleRelativeSearch(await $$xpath(tags, selectHiddenElement), selector.args);
    return get;
};
...
const $$ = async (selector) => {
    const elements = await runtimeHandler.findElements(`document.querySelectorAll('${selector}')`);
    return (await filterVisibleNodes(elements, evaluate));
};
...
const $$xpath = async (selector, selectHiddenElements) => {
    ...
    const elements = await runtimeHandler.findElements(xpathFunc, selector);
    return selectHiddenElements ? elements : (await filterVisibleNodes(elements, evaluate));
};
...
for (const func in module.exports) {
    ...
    if (realFuncs[func].constructor.name === 'AsyncFunction')
        ...
        module.exports[func] = async function () {
            if (config.observe) { await waitFor(config.observeTime); }
