...
let chromeProcess, page, network, runtime, input, client, dom, emulation, overlay, criTarget, currentPort, currentHost, rootId = null, headful;
    ...
    return new Promise(async function connect(resolve) {
        try {
            ...
            await cri({ target }, async (c) => {
                ...
                criTarget = c.Target;
                overlay = c.Overlay;
                await Promise.all([network.enable(), page.enable(), dom.enable(), overlay.enable()]);
 ...
 * @param {Array} [options.args=['--window-size=1440,900']] - Args to open chromium https://peter.sh/experiments/chromium-command-line-switches/.
 * @param {number} [options.port=9222] - Remote debugging port if not given connects to any open port.
...
const highlightElemOnAction = async (elem) => {
    const result = await boundingBox.getBoxModel(dom,elem);
