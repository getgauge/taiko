...
const desc = (selector,query,tag) => {
    let description;
    if(selector.attribute && selector.value) description = `${tag} with attribute "${selector.attribute}" and value "${selector.value}"`;
    else if(selector.label) description = `${tag} with ${query} ${selector.label}`;
    else description = `${tag} matching proximity selectors`;
    return description;
};
...
module.exports.image = (attribute,value, ...args) => {
    ...
    return { get: get, exists: exists(get), description: `Image with "alt=${selector.label}"` };
};
