...
function match(text, ...args) {
    ...
    assertType(text);
    const get = async (e = '*') => {
        let elements = await $$xpath('//' + e + `[translate(normalize-space(@value),"ABCDEFGHIJKLMNOPQRSTUVWXYZ","abcdefghijklmnopqrstuvwxyz")=${xpath(text)} or translate(normalize-space(@type),"ABCDEFGHIJKLMNOPQRSTUVWXYZ","abcdefghijklmnopqrstuvwxyz")=${xpath(text)}]`);
