...
module.exports.openTab = async (
  ...
  if (this.currentContext) {
    const browserContextId = this.currentContext.browserContextId;
    const { targetId } = await _client.Target.createTarget({ url: targetUrl, browserContextId });
    targetHandler.register(options.name, targetId);
    await connect_to_cri(targetId, options);
  } else {
...
module.exports.openIncognitoWindow = async (url, options = {}) => {
  ...
  _client.removeAllListeners();
  const { targetId, browserContextId } = await browserContext.createBrowserContext(url, options);
  await connect_to_cri(targetId);
  targetHandler.register(options.name, { targetId, browserContextId, incognito: true });
  this.currentContext = targetHandler.register(options.name);
  if (url !== 'about:blank') {
...
module.exports.closeIncognitoWindow = async (arg) => {
  ...
  await browserContext.closeBrowserContext(targetHandler.register(arg), connect_to_cri);
...
const _switchContext = async (arg) => {
  ...
  if (targetId) {
    ...
    this.currentContext = targetHandler.register(arg.name);
  } else {
...
module.exports.closeTab = async (identifier) => {
  ...
  let activeTargetIdentifier = false;
  if (!identifier) {
    identifier = activeTarget;
    activeTargetIdentifier = true;
  }
  ...
  let message;
  if (activeTargetIdentifier) {
    message = `Closed current tab matching ${closedTabUrl}`;
  } else {
    if (isObject(identifier) || isString(identifier)) {
      let name = identifier.name || identifier;
      message = `Closed tab(s) matching ${name}` || `Closed tab(s) matching ${name}`;
    }
  }
