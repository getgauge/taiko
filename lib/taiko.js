...
module.exports.switchTo = async arg => {
  ...
  if (isObject(arg)) {
    ...
    if (contexts.size !== 0) {
      ...
      descEvent.emit(
        'success',
        `Switched to Incognito window matching ${arg}`,
      );
    }
  } else {
    if (
...
module.exports.openWindow = async (
  ...
  if (!browserMode && options.incognito) {
    console.warn(
      'Incognito windows in non-headless mode is unstable and may have issues',
    );
  }
  ...
  const targetId = await createBrowserContext(
    browser,
    url,
    runtimeHandler,
    options,
  ...
  options.incognito
    ? descEvent.emit(
        'success',
        `Incognito window opened with name ${options.name}`,
      )
    : descEvent.emit(
        'success',
        `Window opened with name ${options.name}`,
      );
};
...
const _switchToIncognitoBrowser = async arg => {
  ...
  descEvent.emit(
    'success',
    `Switched to Incognito window ${arg.name}`,
  );
};
  ...
  if (!selector.attrValuePairs && !selector.label) {
    ...
    const getTableCell = getElementGetter(
      ...
      async () =>
        ...
        //table[caption=${xpath(selector.label)}]//tr[${
            options.row
          }]/td[${options.col}] | 
        //table[thead[contains(string(),${xpath(
          selector.label,
        )})]]//tr[${options.row}]//td[${options.col}]`,
        ),
      '//table//td',
    );
    get = async () => {
