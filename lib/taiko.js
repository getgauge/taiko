...
module.exports.link = (attrValuePairs, _options = {},...args) => {
  validate();
  const { selector, options } = prepareParameters(attrValuePairs, _options,
      ...args);
  const get = getElementGetter(
    selector,
    async () => await match(selector.label,options).get('a', 0, 0),
    '//a',options.selectHiddenElement
  );
  return {
    get: getIfExists(get),
    exists: exists(getIfExists(get), descEvent),
...
/**
 *
 * Function to select elements which contains specific Text
 *
 * @example
 * await click(containsText('Get Started'))
 * await containsText('Get Started').exist()
 *
 * @param {string} text - The link text.
 * @param args - Proximity selectors
 * @returns {ElementWrapper}
 */
module.exports.containsText = (text, ...args) => {
  validate();
  assertType(text);
  ...
  const get = async () => await handleRelativeSearch(await $$xpath(
...
/**
 *
 * Function to select elements which starts with specific Text
 *
 * @example
 * await click(startsWithText('Get Started'))
 * await startsWithText('Get Started').exist()
 *
 * @param {string} text - The link text.
 * @param args - Proximity selectors
 * @returns {ElementWrapper}
 */
module.exports.startsWithText = (text, ...args) => {
    validate();
    assertType(text);
    let nbspChar = String.fromCharCode(160);
    let textToTranslate = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' + nbspChar;
    ...
    const get = async () => await handleRelativeSearch(await $$xpath(
        `//text()[starts-with(translate(normalize-space(string()), "${textToTranslate}", "${translateTo}"), ${xpathText})]`,
    ),args);
    return {
        get: getIfExists(get),
        exists: exists(getIfExists(get), descEvent),
        description: `Element starts with text "${text}"`,
        text: selectorText(get),
    };
};
...
const addPromiseToWait = promises => {
  return promise => {
    if(promise.hasOwnProperty('request')) {
      ...
      logEvent(`Waiting for:\t RequestId : ${request.requestId}\tRequest Url : ${request.request.url}`);
