...
module.exports.openBrowser = async options => {
    ...
    if(options && options.headless) arguments.push('--headless');
    assert(revisionInfo.local, `Chromium revision is not downloaded. Run "npm install"`);
    chromeExecutable = revisionInfo.executablePath;
    chromeProcess = childProcess.spawn(chromeExecutable,arguments);
...
const scroll = async (e, px, scrollPage, scrollElement, direction) => {
    ...
    if (Number.isInteger(e)) {
        const result = await runtime.evaluate({expression:`(${scrollPage}).apply(null, ${JSON.stringify([e])})`});
...
function match(text, ...args) {
    ...
    const get = async (e = '*') => {
        ...
        if (!elements || !elements.length)
        elements = await $$xpath('//' + e + `[text()=${xpath(text)}]`);
        return await handleRelativeSearch(elements, args);
    };
