...
const checkIfElementAtPointOrChild = async e => {
  function isElementAtPointOrChild() {
    ...
    return (
      elem.contains(node) || node.contains(elem) || 
...
const selectorText = (get, isButton = false) => {
  return async () => {
    const texts = [];
    ...
    function getTextOrValue() {
      return (this.innerText) ? this.innerText : this.value;
    }
    for (const elem of elems) {
      var text = (isButton) ? (await evaluate(elem, getTextOrValue)).value : (await evaluate(elem, getText)).value;
      texts.push(text);
    }
    return texts;
  };
