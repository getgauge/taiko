...
module.exports.image = (attrValuePairs, ...args) => {
  ...
  return {
    get: async (tag, retryInterval, retryTimeout) => {
      ...
      return await getIfExists(get, description)(tag, retryInterval, retryTimeout);
    },
    exists: async () => {
...
module.exports.link = (attrValuePairs, ...args) => {
  ...
  const get = getElementGetter(
    selector,
    async () => await match(selector.label).elements('a', 0, 0),
    '//a',
  );
...
function fileField(attrValuePairs, _options = {}, ...args) {
  ...
  return {
    ...
    description,
    value: async () => {
      const nodeId = await (await getIfExists(get, description)())[0].get();
      if (!nodeId)
