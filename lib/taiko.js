...
eventHandler.addListener('targetCreated', async newTarget => {
  ...
  const pages = browserTargets.filter(target => {
    return target.id === newTarget.targetId;
  });
...
module.exports.openBrowser = async (
  ...
  if (options.host && options.port) {
    ...
    if (!options.headless && options.profiles) {
      throw new Error(
        'Failed to open browser in headfull mode with profiles',
      );
    }
    let args = [
    ...
    browserDebug = endpoint.browser;
  }
  if (options.profiles) {
    browser = await connect_to_cri(browserDebug, options);
  } else {
...
module.exports.openIncognitoBrowser = async (
  profileName = 'default',
) => {
...
module.exports.closeIncognitoBrowser = async (
  profileName = 'default',
) => {
...
module.exports.switchToIncognitoBrowser = async (
  profileName = 'default',
) => {
