...
module.exports.title = async () => {
    ...
    const result = await runtimeHandler.runtimeEvaluate(
        'document.querySelector("title").textContent', null, {returnByValue:true}
    );
...
const checkIfChildOfOtherMatches = async (elem, elements) => {
    ...
    const { object: { objectId } } = await dom.resolveNode({ nodeId: elem });
    const result = await runtime.callFunctionOn({ 
        functionDeclaration: getElementFromPoint.toString(),
        objectId
    });
