...
const { helper, wait, isString, isFunction, getIfExists,
 ...
 *
 * @example
 * link('Plugins').exists(toMilliSeconds(1))
 * waitFor(toMilliSeconds(2))
 *
...
/**
 * @deprecated Use toMilliSeconds API to converts seconds to milliseconds.
 * Converts seconds to milliseconds.
 *
 * @example
...
module.exports.intervalSecs = (secs) => {
    console.warn('DEPRECATION WARNING: intervalSecs is deprecated, use toMilliSeconds to converts seconds to milliseconds.');
...
module.exports.timeoutSecs = (secs) => {
    console.warn('DEPRECATION WARNING: timeoutSecs is deprecated, use toMilliSeconds to converts seconds to milliseconds.');
    return toMilliSeconds(secs);
};
 ...
 *
 * @example
 * waitFor(toMilliSeconds(5))
 * waitFor("1 item in cart")
 * waitFor("Order Created", toMilliSeconds(20))
 *
...
const loadPlugin = (id, init) => {
    try {
        if (!plugins.has(id)) {
            ...
            init(module.exports, xhrEventProxy);
            plugins.set(id, init);
        }
    } catch (error) {
        console.trace(error);
 ...
 * @example
 * link('google').exists()
 * link('google').exists(toMilliSeconds(1))
...
module.exports.metadata = {
    ...
    'Helpers': ['evaluate', 'intervalSecs', 'timeoutSecs', 'toMilliSeconds', 'to', 'into', 'waitFor', 'accept', 'dismiss', 'setConfig', 'emitter'],
