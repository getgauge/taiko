...
/**
 * Clears the value of given selector by setting attribute 'value' to empty string. If no selector is given clears the current active element. 
 * 
 * @example
 * clear()
 ...
 * @returns {Promise<Object>} - Object with the description of the action performed.
 */
module.exports.clear = async (selector) => {
    if(!selector) {
        ...
        return  { description: 'Cleared element on focus'};
    }
    const result = await evaluate(selector, function getvalue() { this.value = ''; return true;});
    if (headful) await highlightElemOnAction(await element(selector));
    if(!result.value) throw new Error('Unable to clear ' + description(selector, true));
    return  { description: 'Cleared ' + description(selector, true) };
};
