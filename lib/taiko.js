...
module.exports.screenshot = async (selector, options = {}) => {
  ...
  _options.path = _options.path || `Screenshot-${Date.now()}.png`;
  const screenShot = await pageHandler.captureScreenshot(
    domHandler,
    selector,
    _options,
  );
  ...
  fs.writeFileSync(_options.path, Buffer.from(screenShot.data, "base64"));
  const e = _options.encoding !== undefined ? _options.encoding : _options.path;
...
// biome-ignore lint/style/useDefaultParameterLast: Require for default value
function tableCell(options = {}, attrValuePairs, ...args) {
  let _attrValuePairs = attrValuePairs;
  let _options = options;
  validate();
  if (_options.row === 0) {
    ...
    async (e, v) => {
      ...
      return (
        [rect.bottom, rect.top].some(
          (offSet) =>
            offSet > v.top - nearOffset && offSet < v.bottom + nearOffset,
        ) &&
        [rect.left, rect.right].some(
          (offSet) =>
            offSet > v.left - nearOffset && offSet < v.right + nearOffset,
        )
      );
    },
 ...
 * @param {function} callback - Action to perform. Accept/Dismiss.
 */
module.exports.beforeunload = (callback) =>
  dialog("beforeunload", "", callback);
...
module.exports.evaluate = async (selector, callback, options = {}) => {
  ...
  async function evalFunc({ callback, args }) {
    ...
    try {
      ...
      throw new Error(`Error evaluating the callback function: ${error.message}`);
    }
...
const waitFor = async (element, time, options = {}) => {
  ...
  if (!element || Number.isFinite(element)) {
    return wait(element);
    // biome-ignore lint/style/noUselessElse: Don't break the chain
  } else if (isString(element)) {
    const foundElements = await match(element).elements(
      undefined,
    ...
    const foundElements = await element.elements(
      defaultConfig.retryInterval,
      timeout,
    );
    if (!foundElements.length) {
...
// biome-ignore lint/complexity/noForEach: Cannot use for of and this is done on load
...
const dialog = (dialogType, dialogMessage, callback) => {
  ...
  const _callback = isFunction(dialogMessage) ? dialogMessage : callback;
  const eventName = isFunction(dialogMessage)
    ? dialogType
    : createJsDialogEvent(dialogMessage, dialogType);
...
for (const func in module.exports) {
  ...
  if (realFuncs[func].constructor.name === "AsyncFunction") {
    module.exports[func] = async function () {
      ...
      // biome-ignore lint/style/noArguments: Cannot use optional arguments
