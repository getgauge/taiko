...
module.exports.checkBox = (attrValuePairs, _options, ...args) => {
    ...
    const get = getElementGetter(selector,
        async () => await $$xpath(
            `//input[@type='checkbox'][@id=(//label[contains(string(), ${xpath(selector.label)})]/@for)] | //label[contains(string(), ${xpath(selector.label)})]/input[@type='checkbox'] | //input[@type='checkbox'][following-sibling::text()[position() = 1 and contains(., ${xpath(selector.label)})]]`,
            options.selectHiddenElement
        ),
        '//input[@type="checkbox"]',
        options.selectHiddenElement
