...
const TableCellWrapper = require('./tableCellWrapper');
...
const createElementWrapper = (elementType, query, attrValuePairs, _options, ...args) => {
  ...
  const description = desc(selector, query, elementType, options);
  switch (elementType) {
    case 'CustomSelector':
    ...
    case 'Element':
      return new TextWrapper(selector, options, description);
    case 'FileField':
    ...
    case 'Table':
      return new TableCellWrapper(selector, options, description);
    default:
