...
const getElementGetter = (selector, query, tags, selectHiddenElements) => {
  ...
  if (selector.attrValuePairs) {
    let query = tags
      ...
      .join('|');
    get = async () =>
      await handleRelativeSearch(await $$xpath(query, selectHiddenElements), selector.args);
  } else if (selector.label) {
    get = async () => await handleRelativeSearch(await query(), selector.args);
  } else {
    get = async () =>
      await handleRelativeSearch(await $$xpath(tags, selectHiddenElements), selector.args);
  }
  return get;
};
