...
const getCriTargets = async function (targetUrl) {
    ...
    if (targetUrl) {
        ...
        for (const target of targets) {
            ...
            const targetUrlPath = path.join(targetHost, trimChar(parsedTargetUrl.pathname,'/'));
            const urlPath = path.join(host, trimChar(parsedUrl.pathname,'/'));

            target.url = handleUrlRedirection(target.url);
            if (target.title === targetUrl || urlPath === (targetUrlPath || targetUrl) || (parsedUrl.href === targetUrl)) {
