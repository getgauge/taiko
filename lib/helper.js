...
const exists = (get, eventHandler) => {
  return async (retryInterval, retryTimeout) => {
    if ((await get(null, retryInterval, retryTimeout)).length) {
...
const isDisabled = (get, eventHandler) => {
  return async (retryInterval, retryTimeout) => {
    const nodeIds = await get(null, retryInterval, retryTimeout);
    for(nodeId of nodeIds){
      ...
      if(result.value){
        eventHandler.emit('success', 'disabled');
        return true;
      }

      function checkDisabled() {
        console.log(this);
        return this.disabled || this.parentElement.disabled;
      }
    }

    eventHandler.emit('success', 'not disabled');
    return false;
  };
};
