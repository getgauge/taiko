...
const isStrictObject = obj =>
  obj && typeof obj === 'object' && (obj.constructor === Object || obj.constructor.name === 'Object');
...
const exists = (n, eventHandler) => {
  if (n) {
    eventHandler.emit('success', 'Exists');
    return true;
  }
  eventHandler.emit('success', 'Does not exist');
  return false;
};

const getIfExists = (get, description, eventEmitter) => {
  return async (tag, retryInterval, retryTimeout) => {
    ...
    try {
      ...
      if(!nodeIds.length) throw new Error();
      await waitUntil(
        async () => nodeIds.length > 0,
        retryInterval,
        retryTimeout,
      );
      function getText(){
      ...
      let elementWrappers = nodeIds.map(nodeId => {
        ...
        return {
          get: async () => nodeId,
          exists: async () => {
            eventEmitter.emit('success','Exists');
            return true;
          },
          text: async () => {
            const result = await runtimeCallFunctionOn(getText, null, {nodeId});
      ...
      return [
        {
          exists: async () => {
            eventEmitter.emit('success','Does not Exists');
            return false;
          }, 
          description, 
          text: async () => ""
