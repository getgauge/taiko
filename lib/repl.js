...
function step(withImports = false, actions = commands) {
    ...
    if (actions[actions.length - 1].includes('closeBrowser()')) actions = actions.slice(0, -1);
    ...
    for (let func in taiko) {
        if (taiko[func].constructor.name === 'AsyncFunction') repl.context[func] = async function () {
            try {
                ...
                const res = await taiko[func].apply(this, args);
            ...
            taikoCommands[func] = true;
            const res = taiko[func].apply(this, arguments);
            if(res.exists) res.exists = async () => {
                const result = await res.exists;
                if(result) return {description:'Exists'};
